...
TEST_CASE("8uC1", "[CUDA.Arithmetic.Mul]")
{
    ...
    cpu_src1.Mul(cpu_src2, cpu_dst);
    gpu_src1.Mul(gpu_src2, gpu_dst);

    gpu_res << gpu_dst;

    CHECK(cpu_dst.IsIdentical(gpu_res));

    cpu_src1.Mul(cpu_src2, cpu_dst, 8);
    gpu_src1.Mul(gpu_src2, gpu_dst, 8);
    ...
    // Inplace
    cpu_dst.Mul(cpu_src2, 3);
    gpu_dst.Mul(gpu_src2, 3);

    gpu_res << gpu_dst;

    CHECK(cpu_dst.IsIdentical(gpu_res));

    cpu_src1.Mul(cpu_src2);
    gpu_src1.Mul(gpu_src2);
...
TEST_CASE("8uC1", "[CUDA.Arithmetic.Mul.UnevenSizeAlloc]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC1", "[CUDA.Arithmetic.Mul.EvenSizeAlloc]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC1", "[CUDA.Arithmetic.Mul.UnalignedPitch]")
{
    gpu::Image<Pixel8uC1> gpu_src1(size, size);
    gpu::Image<Pixel8uC1> gpu_src2(size, size);
    gpu::Image<Pixel8uC1> gpu_dst(size, size);
    ...
    CHECK_THROWS_AS(gpu_src1Pitch.Mul(gpu_src2Pitch, gpu_dstPitch), PitchException);
}

TEST_CASE("8uC1", "[CUDA.Arithmetic.Mul.NullPtr]")
{
    gpu::Image<Pixel8uC1> gpu_src1(size, size);
    gpu::Image<Pixel8uC1> gpu_src2(size, size);
    ...
    CHECK_THROWS_AS(gpu_src1.Mul(gpu_src2, gpu_dst), NullPtrException);
}

TEST_CASE("8uC1", "[CUDA.Arithmetic.Mul.Roi]")
{
    gpu::Image<Pixel8uC1> gpu_src1(size, size);
    gpu::Image<Pixel8uC1> gpu_src2(size, size);
    gpu::Image<Pixel8uC1> gpu_dst(size, size);
    ...
    CHECK_THROWS_AS(gpu_src1.Mul(gpu_src2, gpu_dst), RoiException);
}

TEST_CASE("8uC2", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC3", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC4", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC4A", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
    ...
    // Inplace
    cpu_dst.Mul(cpu_src2, cpu_dst, 3);
    gpu_dst.Mul(gpu_src2, 3);
...
TEST_CASE("16uC1", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC2", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC3", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC4", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC4A", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32sC1", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32sC2", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32sC3", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC1", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
    ...
    // Inplace
    cpu_dst.Mul(cpu_src2);
    gpu_dst.Mul(gpu_src2);
...
TEST_CASE("16fC2", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC3", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC4", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC1", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC2", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC3", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC4", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC4A", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC1", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC2", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC3", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC4", "[CUDA.Arithmetic.Mul]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC1", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
    ...
    cpu_src1.Mul(constVal, cpu_dst);
    gpu_src1.Mul(constVal, gpu_dst);

    gpu_res << gpu_dst;

    CHECK(cpu_dst.IsIdentical(gpu_res));

    cpu_src1.Mul(constVal, cpu_dst, 8);
    gpu_src1.Mul(constVal, gpu_dst, 8);
    ...
    // Inplace
    cpu_dst.Mul(constVal, 3);
    gpu_dst.Mul(constVal, 3);

    gpu_res << gpu_dst;

    CHECK(cpu_dst.IsIdentical(gpu_res));

    cpu_src1.Mul(constVal);
    gpu_src1.Mul(constVal);
...
TEST_CASE("8uC2", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC3", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC4", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC4A", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC1", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC2", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC3", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC4", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC4A", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32sC1", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32sC2", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32sC3", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC1", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
    ...
    // Inplace
    cpu_dst.Mul(constVal);
    gpu_dst.Mul(constVal);
...
TEST_CASE("16fC2", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC3", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC4", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC1", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC2", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC3", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC4", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC4A", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC1", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC2", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC3", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC4", "[CUDA.Arithmetic.MulC]")
{
    const uint seed = Catch::getSeed();
    ...
    // Inplace
    cpu_dst.Mul(constVal, 3);
    gpu_dst.Mul(constVal, gpu_dst, 3);
...
TEST_CASE("8uC1", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC2", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
    ...
    cpu_src1.Mul(constVal, cpu_dst);
    gpu_src1.Mul(constDevVal, gpu_dst);
    ...
    cpu_src1.Mul(constVal, cpu_dst, 8);
    gpu_src1.Mul(constDevVal, gpu_dst, 8);
...
TEST_CASE("8uC3", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC4", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC4A", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC1", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC2", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC3", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC4", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC4A", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32sC1", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32sC2", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32sC3", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC1", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC2", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC3", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC4", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC1", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC2", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC3", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC4", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC4A", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC1", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC2", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC3", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC4", "[CUDA.Arithmetic.MulDevC]")
{
    const uint seed = Catch::getSeed();
    ...
    // Inplace
    cpu_dst.Mul(constVal, 3);
    gpu_dst.Mul(constDevVal, gpu_dst, 3);
