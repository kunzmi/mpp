...
TEST_CASE("8uC1", "[CUDA.Arithmetic.Sub]")
{
    ...
    cpu_src1.Sub(cpu_src2, cpu_dst);
    gpu_src1.Sub(gpu_src2, gpu_dst);

    gpu_res << gpu_dst;

    CHECK(cpu_dst.IsIdentical(gpu_res));

    cpu_src1.Sub(cpu_src2, cpu_dst, -1);
    gpu_src1.Sub(gpu_src2, gpu_dst, -1);
    ...
    // Inplace
    cpu_dst.Sub(cpu_src2, 1);
    gpu_dst.Sub(gpu_src2, 1);

    gpu_res << gpu_dst;

    CHECK(cpu_dst.IsIdentical(gpu_res));

    cpu_src1.Sub(cpu_src2);
    gpu_src1.Sub(gpu_src2);
    ...
    // InplaceInv
    cpu_src1.SubInv(cpu_src2);
    gpu_src1.SubInv(gpu_src2);
...
TEST_CASE("8uC1", "[CUDA.Arithmetic.Sub.UnevenSizeAlloc]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC1", "[CUDA.Arithmetic.Sub.EvenSizeAlloc]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC1", "[CUDA.Arithmetic.Sub.UnalignedPitch]")
{
    gpu::Image<Pixel8uC1> gpu_src1(size, size);
    gpu::Image<Pixel8uC1> gpu_src2(size, size);
    gpu::Image<Pixel8uC1> gpu_dst(size, size);
    ...
    CHECK_THROWS_AS(gpu_src1Pitch.Sub(gpu_src2Pitch, gpu_dstPitch), PitchException);
}

TEST_CASE("8uC1", "[CUDA.Arithmetic.Sub.NullPtr]")
{
    gpu::Image<Pixel8uC1> gpu_src1(size, size);
    gpu::Image<Pixel8uC1> gpu_src2(size, size);
    ...
    CHECK_THROWS_AS(gpu_src1.Sub(gpu_src2, gpu_dst), NullPtrException);
}

TEST_CASE("8uC1", "[CUDA.Arithmetic.Sub.Roi]")
{
    gpu::Image<Pixel8uC1> gpu_src1(size, size);
    gpu::Image<Pixel8uC1> gpu_src2(size, size);
    gpu::Image<Pixel8uC1> gpu_dst(size, size);
    ...
    CHECK_THROWS_AS(gpu_src1.Sub(gpu_src2, gpu_dst), RoiException);
}

TEST_CASE("8uC2", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC3", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC4", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC4A", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
    ...
    // Inplace
    cpu_dst.Sub(cpu_src2, cpu_dst, 1);
    gpu_dst.Sub(gpu_src2, 1);
...
TEST_CASE("16uC1", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC2", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC3", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC4", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC4A", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32sC1", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32sC2", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32sC3", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC1", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
    ...
    // Inplace
    cpu_dst.Sub(cpu_src2);
    gpu_dst.Sub(gpu_src2);
...
TEST_CASE("16fC2", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC3", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC4", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC1", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC2", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC3", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC4", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC4A", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC1", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC2", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC3", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC4", "[CUDA.Arithmetic.Sub]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC1", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
    ...
    cpu_src1.Sub(constVal, cpu_dst);
    gpu_src1.Sub(constVal, gpu_dst);

    gpu_res << gpu_dst;

    CHECK(cpu_dst.IsIdentical(gpu_res));

    cpu_src1.Sub(constVal, cpu_dst, -1);
    gpu_src1.Sub(constVal, gpu_dst, -1);

    gpu_res << gpu_dst;

    CHECK(cpu_dst.IsIdentical(gpu_res));

    cpu_dst.Sub(constVal, 1);
    gpu_dst.Sub(constVal, 1);
    ...
    // Inplace
    cpu_src1.Sub(constVal);
    gpu_src1.Sub(constVal);
    ...
    // InplaceInv
    cpu_src1.SubInv(constVal);
    gpu_src1.SubInv(constVal);
...
TEST_CASE("8uC2", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC3", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC4", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC4A", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC1", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC2", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC3", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC4", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC4A", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32sC1", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32sC2", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32sC3", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC1", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
    ...
    // Inplace
    cpu_dst.Sub(constVal);
    gpu_dst.Sub(constVal);
...
TEST_CASE("16fC2", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC3", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC4", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC1", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC2", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC3", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC4", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC4A", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC1", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC2", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC3", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC4", "[CUDA.Arithmetic.SubC]")
{
    const uint seed = Catch::getSeed();
    ...
    // Inplace
    cpu_dst.Sub(constVal, 1);
    gpu_dst.Sub(constVal, gpu_dst, 1);
...
TEST_CASE("8uC1", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC2", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
    ...
    cpu_src1.Sub(constVal, cpu_dst);
    gpu_src1.Sub(constDevVal, gpu_dst);
    ...
    cpu_src1.Sub(constVal, cpu_dst, -1);
    gpu_src1.Sub(constDevVal, gpu_dst, -1);
...
TEST_CASE("8uC3", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC4", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("8uC4A", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC1", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC2", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC3", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC4", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16uC4A", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32sC1", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32sC2", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32sC3", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC1", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC2", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC3", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16fC4", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC1", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC2", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC3", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC4", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("32fC4A", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC1", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC2", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC3", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
...
TEST_CASE("16scC4", "[CUDA.Arithmetic.SubDevC]")
{
    const uint seed = Catch::getSeed();
    ...
    // Inplace
    cpu_dst.Sub(constVal, 1);
    gpu_dst.Sub(constDevVal, gpu_dst, 1);
