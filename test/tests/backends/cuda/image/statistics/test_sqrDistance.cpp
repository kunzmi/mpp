...
TEST_CASE("8uC1", "[CUDA.Statistics.SquareDistanceNormalized]")
{
    ...
    gpu_src1.SquareDistanceNormalized(gpu_tpl, gpu_dst, {0}, BorderType::Constant, Roi(0, 0, size, size));
    gpu_res << gpu_dst;

    cpu_dst.Set(0);
    cpu_dst.SetRoi(Border(-8));
    cpu_src1.SetRoi(Border(-8));
    cpu_src1.SquareDistanceNormalized(cpu_tpl, cpu_dst, {0}, BorderType::Constant, Roi(0, 0, size, size));
    cpu_dst.ResetRoi();
    gpu_dst.ResetRoi();
    cpu_src1.ResetRoi();
    gpu_src1.ResetRoi();
...
TEST_CASE("16uC1", "[CUDA.Statistics.SquareDistanceNormalized]")
{
    std::filesystem::path root = std::filesystem::path(TEST_DATA_DIR);
...
TEST_CASE("32fC1", "[CUDA.Statistics.SquareDistanceNormalized]")
{
    std::filesystem::path root = std::filesystem::path(TEST_DATA_DIR);
