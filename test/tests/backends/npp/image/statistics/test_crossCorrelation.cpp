...
TEST_CASE("8uC1", "[NPP.Statistics.CrossCorrelation]")
{
    ...
    npp_src1.CrossCorrValid(npp_tpl, npp_dst, nppCtx);
    npp_res << npp_dst;
    ...
    cpu_src1.CrossCorrelation(cpu_tpl, cpu_dst, {0}, BorderType::Constant, Roi(0, 0, size, size));
    cpu_dst.ResetRoi();
    npp_dst.ResetRoi();
...
TEST_CASE("16uC1", "[NPP.Statistics.CrossCorrelation]")
{
    // const uint seed            = Catch::getSeed();
    NppStreamContext nppCtx    = nv::Image8uC1::GetStreamContext();
    std::filesystem::path root = std::filesystem::path(TEST_DATA_DIR);
...
TEST_CASE("32fC1", "[NPP.Statistics.CrossCorrelation]")
{
    // const uint seed            = Catch::getSeed();
    NppStreamContext nppCtx    = nv::Image8uC1::GetStreamContext();
    std::filesystem::path root = std::filesystem::path(TEST_DATA_DIR);
