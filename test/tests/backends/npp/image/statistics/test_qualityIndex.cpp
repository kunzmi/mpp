...
TEST_CASE("8uC1", "[NPP.Statistics.QualityIndex]")
{
    ...
    mpp::cuda::DevVar<byte> npp_buffer(npp_src1.QualityIndexGetBufferHostSize(nppCtx));
    ...
    npp_src1.QualityIndex(npp_src2, npp_src2.NppiSizeRoi(), npp_dst, npp_buffer, nppCtx);
    npp_dst >> npp_res;
...
TEST_CASE("8uC3", "[NPP.Statistics.QualityIndex]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image8uC1::GetStreamContext();
    ...
    // BUG in NPP, channel 1 and 2 are weird...
    CHECK(cpu_dst.x == Approx(npp_res[0]).margin(0.0000001));
    /*CHECK(cpu_dst.y == Approx(npp_res[1]).margin(0.0000001));
    CHECK(cpu_dst.z == Approx(npp_res[2]).margin(0.0000001));*/
}

TEST_CASE("8uC4", "[NPP.Statistics.QualityIndex]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image8uC1::GetStreamContext();

    cpu::Image<Pixel8uC4A> cpu_src1A(size, size);
    cpu::Image<Pixel8uC4A> cpu_src2A(size, size);
    cpu::ImageView<Pixel8uC4> cpu_src1(cpu_src1A);
    cpu::ImageView<Pixel8uC4> cpu_src2(cpu_src2A);
    Pixel64fC4A cpu_dst;
    float npp_res[4];
    nv::Image8uC4 npp_src1(size, size);
    nv::Image8uC4 npp_src2(size, size);
    mpp::cuda::DevVar<float> npp_dst(4);
    mpp::cuda::DevVar<byte> npp_buffer(npp_src1.QualityIndexGetBufferHostSizeA(nppCtx));
    ...
    npp_src1.QualityIndexA(npp_src2, npp_src2.NppiSizeRoi(), npp_dst, npp_buffer, nppCtx);
    npp_dst >> npp_res;

    cpu_src1A.QualityIndex(cpu_src2A, cpu_dst);
...
TEST_CASE("16uC1", "[NPP.Statistics.QualityIndex]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image8uC1::GetStreamContext();
...
TEST_CASE("16uC3", "[NPP.Statistics.QualityIndex]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16uC4", "[NPP.Statistics.QualityIndex]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image8uC1::GetStreamContext();

    cpu::Image<Pixel16uC4A> cpu_src1A(size, size);
    cpu::Image<Pixel16uC4A> cpu_src2A(size, size);
    cpu::ImageView<Pixel16uC4> cpu_src1(cpu_src1A);
    cpu::ImageView<Pixel16uC4> cpu_src2(cpu_src2A);
    Pixel64fC4A cpu_dst;
    float npp_res[4];
...
TEST_CASE("32fC1", "[NPP.Statistics.QualityIndex]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image8uC1::GetStreamContext();
...
TEST_CASE("32fC3", "[NPP.Statistics.QualityIndex]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32fC4", "[NPP.Statistics.QualityIndex]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image8uC1::GetStreamContext();

    cpu::Image<Pixel32fC4A> cpu_src1A(size, size);
    cpu::Image<Pixel32fC4A> cpu_src2A(size, size);
    cpu::ImageView<Pixel32fC4> cpu_src1(cpu_src1A);
    cpu::ImageView<Pixel32fC4> cpu_src2(cpu_src2A);
    Pixel64fC4A cpu_dst;
    float npp_res[4];
    ...
    cpu_src1.FillRandom(seed);
    cpu_src2.FillRandom(seed + 1);
    cpu_src1.Mul({1, 5, 1, 4});
    cpu_src2.Add(cpu_src1);
