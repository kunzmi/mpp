...
TEST_CASE("8uC1", "[NPP.Morpholgy.GrayErode]")
{
    ...
    npp_src1.GrayErodeBorder(npp_dst, mask, {size_tpl, size_tpl}, {2, 2}, NPP_BORDER_REPLICATE, nppCtx);
    npp_res << npp_dst;

    cpu_src1.ErosionGray(cpu_dst, cpu_tpl2.Pointer(), size_tpl, BorderType::Replicate);

    CHECK(cpu_dst.IsIdentical(npp_res));
}

TEST_CASE("32fC1", "[NPP.Morpholgy.GrayErode]")
{
    std::filesystem::path root = std::filesystem::path(TEST_DATA_DIR);
    const uint seed            = Catch::getSeed();
    NppStreamContext nppCtx    = nv::Image8uC1::GetStreamContext();
