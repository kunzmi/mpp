...
TEST_CASE("8uC1", "[NPP.Morpholgy.TopHat]")
{
    ...
    npp_src1.MorphTopHatBorder(npp_dst, mask, {size_tpl, size_tpl}, {2, 2}, buffer, NPP_BORDER_REPLICATE, nppCtx);
    npp_res << npp_dst;

    cpu_src1.TopHat(cpu_temp, cpu_dst, cpu_tpl.Pointer(), size_tpl, BorderType::Replicate);

    // The result differs, as NPP underflows in the subtraction part
    // CHECK(cpu_dst.IsIdentical(npp_res));
}

TEST_CASE("8uC3", "[NPP.Morpholgy.TopHat]")
{
    std::filesystem::path root = std::filesystem::path(TEST_DATA_DIR);
    const uint seed            = Catch::getSeed();
    NppStreamContext nppCtx    = nv::Image8uC1::GetStreamContext();
...
TEST_CASE("8uC4", "[NPP.Morpholgy.TopHat]")
{
    std::filesystem::path root = std::filesystem::path(TEST_DATA_DIR);
    const uint seed            = Catch::getSeed();
    NppStreamContext nppCtx    = nv::Image8uC1::GetStreamContext();
...
TEST_CASE("16uC1", "[NPP.Morpholgy.TopHat]")
{
    std::filesystem::path root = std::filesystem::path(TEST_DATA_DIR);
    const uint seed            = Catch::getSeed();
    NppStreamContext nppCtx    = nv::Image8uC1::GetStreamContext();
...
TEST_CASE("32fC1", "[NPP.Morpholgy.TopHat]")
{
    std::filesystem::path root = std::filesystem::path(TEST_DATA_DIR);
    const uint seed            = Catch::getSeed();
    NppStreamContext nppCtx    = nv::Image8uC1::GetStreamContext();
...
TEST_CASE("32fC3", "[NPP.Morpholgy.TopHat]")
{
    std::filesystem::path root = std::filesystem::path(TEST_DATA_DIR);
    const uint seed            = Catch::getSeed();
    NppStreamContext nppCtx    = nv::Image8uC1::GetStreamContext();
...
TEST_CASE("32fC4", "[NPP.Morpholgy.TopHat]")
{
    std::filesystem::path root = std::filesystem::path(TEST_DATA_DIR);
    const uint seed            = Catch::getSeed();
    NppStreamContext nppCtx    = nv::Image8uC1::GetStreamContext();
