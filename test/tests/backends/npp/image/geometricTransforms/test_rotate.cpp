...
TEST_CASE("8uC3", "[NPP.GeometricTransforms.Rotate]")
{
    ...
    cpu::Image<Pixel8uC1> cpu_mask = cpu::Image<Pixel8uC1>::Load(root / "knownErrorsNPProt30.tif");
    ...
    AffineTransformation<double> shift1 = AffineTransformation<double>::GetTranslation({-128, -128});
    AffineTransformation<double> rot    = AffineTransformation<double>::GetRotation(30);
    AffineTransformation<double> shift2 = AffineTransformation<double>::GetTranslation({128, 128});
    ...
    npp_src1.Rotate(npp_dst, 30, shift.x, shift.y, static_cast<int>(NPPI_INTER_NN), nppCtx);
    npp_res << npp_dst;

    cpu_dst.Set({128, 0, 0});
    cpu_src1.Rotate(cpu_dst, 30.0, shift, InterpolationMode::NearestNeighbor, BorderType::None);
    ...
    npp_dst.Set({128, 0, 0}, nppCtx);
    npp_src1.Rotate(npp_dst, 30, shift.x, shift.y, static_cast<int>(NPPI_INTER_LINEAR), nppCtx);
    npp_res << npp_dst;

    cpu_dst.Set({128, 0, 0});

    cpu_src1.Rotate(cpu_dst, 30.0, shift, InterpolationMode::Linear, BorderType::None);
    ...
    // cubic interpolation
    npp_dst.Set({128, 0, 0}, nppCtx);
    npp_src1.Rotate(npp_dst, 30, shift.x, shift.y, static_cast<int>(NPPI_INTER_CUBIC), nppCtx);
    npp_res << npp_dst;

    cpu_dst.Set({128, 0, 0});

    cpu_src1.Rotate(cpu_dst, 30.0, shift, InterpolationMode::CubicLagrange, BorderType::None);
...
TEST_CASE("8uP3", "[NPP.GeometricTransforms.Rotate]")
{
    ...
    // NearestNeighbor interpolation
    npp_dst.Set({128, 0, 0}, nppCtx);
    npp_src1.Rotate(npp_dst, 30.0, shift.x, shift.y, static_cast<int>(NPPI_INTER_NN), nppCtx);
    npp_res << npp_dst;
    ...
    cpu::Image<Pixel8uC3>::Rotate(cpu_srcC1, cpu_srcC2, cpu_srcC3, cpu_dstC1, cpu_dstC2, cpu_dstC3, 30.0, shift,
