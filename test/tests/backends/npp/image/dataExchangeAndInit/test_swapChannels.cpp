...
TEST_CASE("8uC3", "[NPP.DataExchangeAndInit.SwapChannels]")
{
    ...
    cpu::Image<Pixel8uC4> cpu_dst4A(size, size);
    ...
    npp_src1.SwapChannels(npp_dst4, order4, 127, nppCtx);
    npp_src1.SwapChannels(npp_dst4A, order4A, 127, nppCtx);
    npp_src1.SwapChannels(npp_dst3, order3, nppCtx);
    npp_src1.SwapChannels(order3, nppCtx);
    npp_res3 << npp_dst3;
    npp_res4 << npp_dst4;
    npp_res4A << npp_dst4A;

    cpu_src1.SwapChannel(cpu_dst3, order3);
    cpu_src1.SwapChannel(cpu_dst4, order4, 127);
    cpu_src1.SwapChannel(cpu_dst4A, order4A, 127);

    CHECK(cpu_dst3.IsIdentical(npp_res3));
    CHECK(cpu_dst4.IsIdentical(npp_res4));
    CHECK(cpu_dst4A.IsIdentical(npp_res4A));
}

TEST_CASE("16sC3", "[NPP.DataExchangeAndInit.SwapChannels]")
{
    const uint seed         = Catch::getSeed();
    ...
    cpu::Image<Pixel16sC4> cpu_dst4A(size, size);
...
TEST_CASE("16uC3", "[NPP.DataExchangeAndInit.SwapChannels]")
{
    const uint seed         = Catch::getSeed();
    ...
    cpu::Image<Pixel16uC4> cpu_dst4A(size, size);
...
TEST_CASE("32sC3", "[NPP.DataExchangeAndInit.SwapChannels]")
{
    const uint seed         = Catch::getSeed();
    ...
    cpu::Image<Pixel32sC4> cpu_dst4A(size, size);
...
TEST_CASE("32fC3", "[NPP.DataExchangeAndInit.SwapChannels]")
{
    const uint seed         = Catch::getSeed();
    ...
    cpu::Image<Pixel32fC4> cpu_dst4A(size, size);
