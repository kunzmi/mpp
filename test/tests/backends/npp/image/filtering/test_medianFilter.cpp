...
TEST_CASE("8uC1", "[NPP.Filtering.MedianFilter]")
{
    ...
    cpu_src1 >> npp_src1;
    mpp::cuda::DevVar<byte> buffer(npp_src1.FilterMedianBorderGetBufferSize({7, 7}, NPP_BORDER_REPLICATE, nppCtx));
    npp_src1.FilterMedianBorder(npp_dst, {3, 3}, {1, 1}, buffer, NPP_BORDER_REPLICATE, nppCtx);
    npp_res << npp_dst;

    cpu_src1.MedianFilter(cpu_dst, 3, BorderType::Replicate);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.FilterMedianBorder(npp_dst, {5, 5}, {2, 2}, buffer, NPP_BORDER_REPLICATE, nppCtx);
    npp_res << npp_dst;

    cpu_src1.MedianFilter(cpu_dst, 5, BorderType::Replicate);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.FilterMedianBorder(npp_dst, {7, 7}, {3, 3}, buffer, NPP_BORDER_REPLICATE, nppCtx);
    npp_res << npp_dst;

    cpu_src1.MedianFilter(cpu_dst, 7, BorderType::Replicate);

    CHECK(cpu_dst.IsIdentical(npp_res));
}

TEST_CASE("8uC3", "[NPP.Filtering.MedianFilter]")
{
    const uint seed         = Catch::getSeed();
    ...
    cpu_src1 >> npp_src1;
    mpp::cuda::DevVar<byte> buffer(npp_src1.FilterMedianBorderGetBufferSize({3, 3}, NPP_BORDER_REPLICATE, nppCtx));
    npp_src1.FilterMedianBorder(npp_dst, {3, 3}, {1, 1}, buffer, NPP_BORDER_REPLICATE, nppCtx);
    npp_res << npp_dst;
...
TEST_CASE("8uC4", "[NPP.Filtering.MedianFilter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16uC1", "[NPP.Filtering.MedianFilter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16uC3", "[NPP.Filtering.MedianFilter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16uC4", "[NPP.Filtering.MedianFilter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16sC1", "[NPP.Filtering.MedianFilter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16sC3", "[NPP.Filtering.MedianFilter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16sC4", "[NPP.Filtering.MedianFilter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32fC1", "[NPP.Filtering.MedianFilter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32fC3", "[NPP.Filtering.MedianFilter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32fC4", "[NPP.Filtering.MedianFilter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32fC4A", "[NPP.Filtering.MedianFilter]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image32fC4::GetStreamContext();
    ...
    cpu_src1A.MedianFilter(cpu_dst, 3, BorderType::Replicate);
