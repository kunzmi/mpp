...
TEST_CASE("8uC1", "[NPP.Filtering.Filter]")
{
    ...
    cpu::Image<Pixel32fC1> filter(5, 5);
    mpp::cuda::DevVar<float> filterd(5 * 5);
    ...
    npp_src1.FilterBorder32f(npp_dst, filterd, {5, 5}, {2, 2}, NPP_BORDER_REPLICATE, nppCtx);
    npp_res << npp_dst;

    cpu_src1.Filter(cpu_dst, reinterpret_cast<float *>(filter.Pointer()), 5, BorderType::Replicate);

    // NPP doesn't round when converting float to int
    CHECK(cpu_dst.IsSimilar(npp_res, 1));
}

TEST_CASE("8uC3", "[NPP.Filtering.Filter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("8uC4", "[NPP.Filtering.Filter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16uC1", "[NPP.Filtering.Filter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16uC3", "[NPP.Filtering.Filter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16uC4", "[NPP.Filtering.Filter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16sC1", "[NPP.Filtering.Filter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16sC3", "[NPP.Filtering.Filter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16sC4", "[NPP.Filtering.Filter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32fC1", "[NPP.Filtering.Filter]")
{
    const uint seed         = Catch::getSeed();
    ...
    npp_src1.FilterBorder(npp_dst, filterd, {5, 5}, {2, 2}, NPP_BORDER_REPLICATE, nppCtx);
    npp_res << npp_dst;
...
TEST_CASE("32fC3", "[NPP.Filtering.Filter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32fC4", "[NPP.Filtering.Filter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32fC4A", "[NPP.Filtering.Filter]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image32fC4::GetStreamContext();
    ...
    cpu_src1A.Filter(cpu_dst, reinterpret_cast<float *>(filter.Pointer()), 5, BorderType::Replicate);
