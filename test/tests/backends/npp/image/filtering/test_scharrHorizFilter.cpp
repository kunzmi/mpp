...
TEST_CASE("8uC1", "[NPP.Filtering.ScharrHorizFilter]")
{
    ...
    npp_src1.FilterScharrHorizBorder(npp_dst, NPP_BORDER_REPLICATE, nppCtx);
    npp_res << npp_dst;

    cpu_src1.FixedFilter(cpu_dst, FixedFilter::ScharrHoriz, MaskSize::Mask_3x3, BorderType::Replicate);

    CHECK(cpu_dst.IsIdentical(npp_res));
}

TEST_CASE("8sC1", "[NPP.Filtering.ScharrHorizFilter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32fC1", "[NPP.Filtering.ScharrHorizFilter]")
{
    const uint seed         = Catch::getSeed();
