...
TEST_CASE("8uC1", "[NPP.Filtering.SobelVertFilter]")
{
    ...
    npp_src1.FilterSobelVertBorder(npp_dst, NppiMaskSize::NPP_MASK_SIZE_3_X_3, NPP_BORDER_REPLICATE, nppCtx);
    npp_res << npp_dst;

    cpu_src1.FixedFilter(cpu_dst, FixedFilter::SobelVert, MaskSize::Mask_3x3, BorderType::Replicate);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.FilterSobelVertBorder(npp_dst, NppiMaskSize::NPP_MASK_SIZE_5_X_5, NPP_BORDER_REPLICATE, nppCtx);
    npp_res << npp_dst;

    cpu_src1.FixedFilter(cpu_dst, FixedFilter::SobelVert, MaskSize::Mask_5x5, BorderType::Replicate);

    CHECK(cpu_dst.IsIdentical(npp_res));
}

TEST_CASE("16sC1", "[NPP.Filtering.SobelVertFilter]")
{
    const uint seed         = Catch::getSeed();
    ...
    npp_src1.FilterSobelVertBorder(npp_dst, NPP_BORDER_REPLICATE, nppCtx);
    npp_res << npp_dst;
...
TEST_CASE("16sC3", "[NPP.Filtering.SobelVertFilter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16sC4", "[NPP.Filtering.SobelVertFilter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32fC1", "[NPP.Filtering.SobelVertFilter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32fC3", "[NPP.Filtering.SobelVertFilter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32fC4", "[NPP.Filtering.SobelVertFilter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32fC4A", "[NPP.Filtering.SobelVertFilter]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image32fC4::GetStreamContext();
    ...
    cpu_src1A.FixedFilter(cpu_dst, FixedFilter::SobelVert, MaskSize::Mask_3x3, BorderType::Replicate);
