...
TEST_CASE("8uC1", "[NPP.Filtering.SobelVertSecondFilter]")
{
    ...
    npp_src1.FilterSobelVertSecondBorder(npp_dst, NPP_MASK_SIZE_3_X_3, NPP_BORDER_REPLICATE, nppCtx);
    npp_res << npp_dst;

    cpu_src1.FixedFilter(cpu_dst, FixedFilter::SobelVertSecond, MaskSize::Mask_3x3, BorderType::Replicate);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.FilterSobelVertSecondBorder(npp_dst, NPP_MASK_SIZE_5_X_5, NPP_BORDER_REPLICATE, nppCtx);
    npp_res << npp_dst;

    cpu_src1.FixedFilter(cpu_dst, FixedFilter::SobelVertSecond, MaskSize::Mask_5x5, BorderType::Replicate);

    CHECK(cpu_dst.IsIdentical(npp_res));
}

TEST_CASE("8sC1", "[NPP.Filtering.SobelVertSecondFilter]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32fC1", "[NPP.Filtering.SobelVertSecondFilter]")
{
    const uint seed         = Catch::getSeed();
