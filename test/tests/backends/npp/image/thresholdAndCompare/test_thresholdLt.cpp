...
TEST_CASE("8uC1", "[NPP.Statistics.ThresholdLt]")
{
    ...
    npp_src1.Threshold_LT(npp_dst, 127, nppCtx);
    npp_res << npp_dst;

    cpu_src1.ThresholdLT(127, cpu_dst);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_LT(127, nppCtx);
    npp_res << npp_src1;

    cpu_src1.ThresholdLT(127);

    CHECK(cpu_src1.IsIdentical(npp_res));
}

TEST_CASE("8uC3", "[NPP.Statistics.ThresholdLt]")
{
    const uint seed         = Catch::getSeed();
    ...
    npp_src1.Threshold_LT(npp_dst, {127, 120, 130}, nppCtx);
    npp_res << npp_dst;

    cpu_src1.ThresholdLT({127, 120, 130}, cpu_dst);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_LT({127, 120, 130}, nppCtx);
    npp_res << npp_src1;

    cpu_src1.ThresholdLT({127, 120, 130});

    CHECK(cpu_src1.IsIdentical(npp_res));
}

TEST_CASE("8uC4A", "[NPP.Statistics.ThresholdLt]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image8uC3::GetStreamContext();
    ...
    npp_src1.Threshold_LTA(npp_dst, {127, 120, 130}, nppCtx);
    npp_res << npp_dst;

    cpu_src1A.ThresholdLT({127, 120, 130}, cpu_dstA);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_LTA({127, 120, 130}, nppCtx);
    npp_res << npp_src1;

    cpu_src1A.ThresholdLT({127, 120, 130});

    CHECK(cpu_src1.IsIdentical(npp_res));
}

TEST_CASE("16sC1", "[NPP.Statistics.ThresholdLt]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16sC3", "[NPP.Statistics.ThresholdLt]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16sC4A", "[NPP.Statistics.ThresholdLt]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image16sC3::GetStreamContext();
...
TEST_CASE("16uC1", "[NPP.Statistics.ThresholdLt]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16uC3", "[NPP.Statistics.ThresholdLt]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16uC4A", "[NPP.Statistics.ThresholdLt]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image16uC3::GetStreamContext();
...
TEST_CASE("32fC1", "[NPP.Statistics.ThresholdLt]")
{
    const uint seed         = Catch::getSeed();
    ...
    npp_src1.Threshold_LT(npp_dst, 0.5f, nppCtx);
    npp_res << npp_dst;

    cpu_src1.ThresholdLT(0.5f, cpu_dst);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_LT(0.5f, nppCtx);
    npp_res << npp_src1;

    cpu_src1.ThresholdLT(0.5f);

    CHECK(cpu_src1.IsIdentical(npp_res));
}

TEST_CASE("32fC3", "[NPP.Statistics.ThresholdLt]")
{
    const uint seed         = Catch::getSeed();
    ...
    npp_src1.Threshold_LT(npp_dst, {0.5f, 0.55f, 0.45f}, nppCtx);
    npp_res << npp_dst;

    cpu_src1.ThresholdLT({0.5f, 0.55f, 0.45f}, cpu_dst);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_LT({0.5f, 0.55f, 0.45f}, nppCtx);
    npp_res << npp_src1;

    cpu_src1.ThresholdLT({0.5f, 0.55f, 0.45f});

    CHECK(cpu_src1.IsIdentical(npp_res));
}

TEST_CASE("32fC4A", "[NPP.Statistics.ThresholdLt]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image32fC3::GetStreamContext();
    ...
    npp_src1.Threshold_LTA(npp_dst, {0.5f, 0.55f, 0.45f}, nppCtx);
    npp_res << npp_dst;

    cpu_src1A.ThresholdLT({0.5f, 0.55f, 0.45f}, cpu_dstA);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_LTA({0.5f, 0.55f, 0.45f}, nppCtx);
    npp_res << npp_src1;

    cpu_src1A.ThresholdLT({0.5f, 0.55f, 0.45f});
