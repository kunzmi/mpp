...
TEST_CASE("8uC1", "[NPP.Statistics.ThresholdGtVal]")
{
    ...
    npp_src1.Threshold_GTVal(npp_dst, 127, 12, nppCtx);
    npp_res << npp_dst;

    cpu_src1.ThresholdGT(127, 12, cpu_dst);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_GTVal(127, 12, nppCtx);
    npp_res << npp_src1;

    cpu_src1.ThresholdGT(127, 12);

    CHECK(cpu_src1.IsIdentical(npp_res));
}

TEST_CASE("8uC3", "[NPP.Statistics.ThresholdGtVal]")
{
    const uint seed         = Catch::getSeed();
    ...
    npp_src1.Threshold_GTVal(npp_dst, {127, 120, 130}, {255, 255, 255}, nppCtx);
    npp_res << npp_dst;

    cpu_src1.ThresholdGT({127, 120, 130}, {255, 255, 255}, cpu_dst);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_GTVal({127, 120, 130}, {255, 255, 255}, nppCtx);
    npp_res << npp_src1;

    cpu_src1.ThresholdGT({127, 120, 130}, {255, 255, 255});

    CHECK(cpu_src1.IsIdentical(npp_res));
}

TEST_CASE("8uC4A", "[NPP.Statistics.ThresholdGtVal]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image8uC3::GetStreamContext();
    ...
    npp_src1.Threshold_GTValA(npp_dst, {127, 120, 130}, {255, 255, 255}, nppCtx);
    npp_res << npp_dst;

    cpu_src1A.ThresholdGT({127, 120, 130}, {255, 255, 255}, cpu_dstA);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_GTValA({127, 120, 130}, {255, 255, 255}, nppCtx);
    npp_res << npp_src1;

    cpu_src1A.ThresholdGT({127, 120, 130}, {255, 255, 255});

    CHECK(cpu_src1.IsIdentical(npp_res));
}

TEST_CASE("16sC1", "[NPP.Statistics.ThresholdGtVal]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16sC3", "[NPP.Statistics.ThresholdGtVal]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16sC4A", "[NPP.Statistics.ThresholdGtVal]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image16sC3::GetStreamContext();
...
TEST_CASE("16uC1", "[NPP.Statistics.ThresholdGtVal]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16uC3", "[NPP.Statistics.ThresholdGtVal]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16uC4A", "[NPP.Statistics.ThresholdGtVal]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image16uC3::GetStreamContext();
...
TEST_CASE("32fC1", "[NPP.Statistics.ThresholdGtVal]")
{
    const uint seed         = Catch::getSeed();
    ...
    npp_src1.Threshold_GTVal(npp_dst, 0.5f, 12, nppCtx);
    npp_res << npp_dst;

    cpu_src1.ThresholdGT(0.5f, 12, cpu_dst);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_GTVal(0.5f, 12, nppCtx);
    npp_res << npp_src1;

    cpu_src1.ThresholdGT(0.5f, 12);

    CHECK(cpu_src1.IsIdentical(npp_res));
}

TEST_CASE("32fC3", "[NPP.Statistics.ThresholdGtVal]")
{
    const uint seed         = Catch::getSeed();
    ...
    npp_src1.Threshold_GTVal(npp_dst, {0.5f, 0.55f, 0.45f}, {1.5f, 1.55f, 1.45f}, nppCtx);
    npp_res << npp_dst;

    cpu_src1.ThresholdGT({0.5f, 0.55f, 0.45f}, {1.5f, 1.55f, 1.45f}, cpu_dst);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_GTVal({0.5f, 0.55f, 0.45f}, {1.5f, 1.55f, 1.45f}, nppCtx);
    npp_res << npp_src1;

    cpu_src1.ThresholdGT({0.5f, 0.55f, 0.45f}, {1.5f, 1.55f, 1.45f});

    CHECK(cpu_src1.IsIdentical(npp_res));
}

TEST_CASE("32fC4A", "[NPP.Statistics.ThresholdGtVal]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image32fC3::GetStreamContext();
    ...
    npp_src1.Threshold_GTValA(npp_dst, {0.5f, 0.55f, 0.45f}, {1.5f, 1.55f, 1.45f}, nppCtx);
    npp_res << npp_dst;

    cpu_src1A.ThresholdGT({0.5f, 0.55f, 0.45f}, {1.5f, 1.55f, 1.45f}, cpu_dstA);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_GTValA({0.5f, 0.55f, 0.45f}, {1.5f, 1.55f, 1.45f}, nppCtx);
    npp_res << npp_src1;

    cpu_src1A.ThresholdGT({0.5f, 0.55f, 0.45f}, {1.5f, 1.55f, 1.45f});
