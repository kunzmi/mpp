...
TEST_CASE("8uC1", "[NPP.Statistics.ThresholdVal]")
{
    ...
    npp_src1.Threshold_Val(npp_dst, 127, 1, NPP_CMP_LESS, nppCtx);
    npp_res << npp_dst;

    cpu_src1.Threshold(127, 1, CompareOp::Less, cpu_dst);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_Val(npp_dst, 127, 1, NPP_CMP_GREATER, nppCtx);
    npp_res << npp_dst;

    cpu_src1.Threshold(127, 1, CompareOp::Greater, cpu_dst);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_Val(127, 1, NPP_CMP_GREATER, nppCtx);
    npp_res << npp_src1;

    cpu_src1.Threshold(127, 1, CompareOp::Greater);

    CHECK(cpu_src1.IsIdentical(npp_res));
}

TEST_CASE("8uC3", "[NPP.Statistics.ThresholdVal]")
{
    const uint seed         = Catch::getSeed();
    ...
    npp_src1.Threshold_Val(npp_dst, {127, 120, 130}, {1, 2, 3}, NPP_CMP_LESS, nppCtx);
    npp_res << npp_dst;

    cpu_src1.Threshold({127, 120, 130}, {1, 2, 3}, CompareOp::Less, cpu_dst);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_Val(npp_dst, {127, 120, 130}, {1, 2, 3}, NPP_CMP_GREATER, nppCtx);
    npp_res << npp_dst;

    cpu_src1.Threshold({127, 120, 130}, {1, 2, 3}, CompareOp::Greater, cpu_dst);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_Val({127, 120, 130}, {1, 2, 3}, NPP_CMP_GREATER, nppCtx);
    npp_res << npp_src1;

    cpu_src1.Threshold({127, 120, 130}, {1, 2, 3}, CompareOp::Greater);

    CHECK(cpu_src1.IsIdentical(npp_res));
}

TEST_CASE("8uC4A", "[NPP.Statistics.ThresholdVal]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image8uC3::GetStreamContext();
    ...
    npp_src1.Threshold_ValA(npp_dst, {127, 120, 130}, {1, 2, 3}, NPP_CMP_LESS, nppCtx);
    npp_res << npp_dst;

    cpu_src1A.Threshold({127, 120, 130}, {1, 2, 3}, CompareOp::Less, cpu_dstA);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_ValA(npp_dst, {127, 120, 130}, {1, 2, 3}, NPP_CMP_GREATER, nppCtx);
    npp_res << npp_dst;

    cpu_src1A.Threshold({127, 120, 130}, {1, 2, 3}, CompareOp::Greater, cpu_dstA);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_ValA({127, 120, 130}, {1, 2, 3}, NPP_CMP_GREATER, nppCtx);
    npp_res << npp_src1;

    cpu_src1A.Threshold({127, 120, 130}, {1, 2, 3}, CompareOp::Greater);

    CHECK(cpu_src1.IsIdentical(npp_res));
}

TEST_CASE("16sC1", "[NPP.Statistics.ThresholdVal]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16sC3", "[NPP.Statistics.ThresholdVal]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16sC4A", "[NPP.Statistics.ThresholdVal]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image16sC3::GetStreamContext();
...
TEST_CASE("16uC1", "[NPP.Statistics.ThresholdVal]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16uC3", "[NPP.Statistics.ThresholdVal]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16uC4A", "[NPP.Statistics.ThresholdVal]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image16uC3::GetStreamContext();
...
TEST_CASE("32fC1", "[NPP.Statistics.ThresholdVal]")
{
    const uint seed         = Catch::getSeed();
    ...
    npp_src1.Threshold_Val(npp_dst, 0.5f, 0.1f, NPP_CMP_LESS, nppCtx);
    npp_res << npp_dst;

    cpu_src1.Threshold(0.5f, 0.1f, CompareOp::Less, cpu_dst);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_Val(npp_dst, 0.5f, 0.1f, NPP_CMP_GREATER, nppCtx);
    npp_res << npp_dst;

    cpu_src1.Threshold(0.5f, 0.1f, CompareOp::Greater, cpu_dst);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_Val(0.5f, 0.1f, NPP_CMP_GREATER, nppCtx);
    npp_res << npp_src1;

    cpu_src1.Threshold(0.5f, 0.1f, CompareOp::Greater);

    CHECK(cpu_src1.IsIdentical(npp_res));
}

TEST_CASE("32fC3", "[NPP.Statistics.ThresholdVal]")
{
    const uint seed         = Catch::getSeed();
    ...
    npp_src1.Threshold_Val(npp_dst, {0.5f, 0.55f, 0.45f}, {0.1f, 0.2f, 0.3f}, NPP_CMP_LESS, nppCtx);
    npp_res << npp_dst;

    cpu_src1.Threshold({0.5f, 0.55f, 0.45f}, {0.1f, 0.2f, 0.3f}, CompareOp::Less, cpu_dst);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_Val(npp_dst, {0.5f, 0.55f, 0.45f}, {0.1f, 0.2f, 0.3f}, NPP_CMP_GREATER, nppCtx);
    npp_res << npp_dst;

    cpu_src1.Threshold({0.5f, 0.55f, 0.45f}, {0.1f, 0.2f, 0.3f}, CompareOp::Greater, cpu_dst);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_Val({0.5f, 0.55f, 0.45f}, {0.1f, 0.2f, 0.3f}, NPP_CMP_GREATER, nppCtx);
    npp_res << npp_src1;

    cpu_src1.Threshold({0.5f, 0.55f, 0.45f}, {0.1f, 0.2f, 0.3f}, CompareOp::Greater);

    CHECK(cpu_src1.IsIdentical(npp_res));
}

TEST_CASE("32fC4A", "[NPP.Statistics.ThresholdVal]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image32fC3::GetStreamContext();
    ...
    npp_src1.Threshold_ValA(npp_dst, {0.5f, 0.55f, 0.45f}, {0.1f, 0.2f, 0.3f}, NPP_CMP_LESS, nppCtx);
    npp_res << npp_dst;

    cpu_src1A.Threshold({0.5f, 0.55f, 0.45f}, {0.1f, 0.2f, 0.3f}, CompareOp::Less, cpu_dstA);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_ValA(npp_dst, {0.5f, 0.55f, 0.45f}, {0.1f, 0.2f, 0.3f}, NPP_CMP_GREATER, nppCtx);
    npp_res << npp_dst;

    cpu_src1A.Threshold({0.5f, 0.55f, 0.45f}, {0.1f, 0.2f, 0.3f}, CompareOp::Greater, cpu_dstA);

    CHECK(cpu_dst.IsIdentical(npp_res));

    npp_src1.Threshold_ValA({0.5f, 0.55f, 0.45f}, {0.1f, 0.2f, 0.3f}, NPP_CMP_GREATER, nppCtx);
    npp_res << npp_src1;

    cpu_src1A.Threshold({0.5f, 0.55f, 0.45f}, {0.1f, 0.2f, 0.3f}, CompareOp::Greater);
