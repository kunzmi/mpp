...
TEST_CASE("8uC1", "[NPP.Arithmetic.Sub]")
{
    ...
    npp_src2.Sub(npp_src1, npp_dst, 0, nppCtx);

    npp_res << npp_dst;

    CHECK(cpu_dst.IsIdentical(npp_res));

    cpu_src1.Sub(cpu_src2, cpu_dst, 2);
    npp_src2.Sub(npp_src1, npp_dst, 2, nppCtx);
...
TEST_CASE("8uC3", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("8uC4", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("8uC4A", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image8uC4::GetStreamContext();
    ...
    cpu_src1A.Sub(cpu_src2A, cpu_dstA);
    npp_src2.SubA(npp_src1, npp_dst, 0, nppCtx);

    npp_res << npp_dst;

    CHECK(cpu_dst.IsIdentical(npp_res));

    cpu_src1A.Sub(cpu_src2A, cpu_dstA, 2);
    npp_src2.SubA(npp_src1, npp_dst, 2, nppCtx);

    npp_res << npp_dst;

    CHECK(cpu_dst.IsIdentical(npp_res));

    cpu_dstA.Sub(cpu_src2A, cpu_dstA, 1);
    npp_dst.SubA(npp_src2, 1, nppCtx);
...
TEST_CASE("16uC1", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16uC3", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16uC4", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16uC4A", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image16uC4::GetStreamContext();
...
TEST_CASE("32sC1", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32sC3", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16fC1", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
    ...
    cpu_src1.Sub(cpu_src2, cpu_dst);
    npp_src2.Sub(npp_src1, npp_dst, nppCtx);
...
TEST_CASE("16fC3", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16fC4", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32fC1", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32fC3", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32fC4", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("32fC4A", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image32fC4::GetStreamContext();
    ...
    cpu_src1A.Sub(cpu_src2A, cpu_dstA);
    npp_src2.SubA(npp_src1, npp_dst, nppCtx);

    npp_res << npp_dst;

    CHECK(cpu_dst.IsIdentical(npp_res));

    cpu_dstA.Sub(cpu_src2A, cpu_dstA);
    npp_dst.SubA(npp_src2, nppCtx);
...
TEST_CASE("16scC1", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16scC3", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("16scC4", "[NPP.Arithmetic.Sub]")
{
    const uint seed         = Catch::getSeed();
...
TEST_CASE("8uC1", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image8uC1::GetStreamContext();
    ...
    npp_src1.Sub(constVal, npp_dst, 0, nppCtx);

    npp_res << npp_dst;

    CHECK(cpu_dst.IsIdentical(npp_res));

    cpu_src1.Sub(constVal, cpu_dst, 2);
    npp_src1.Sub(constVal, npp_dst, 2, nppCtx);

    npp_res << npp_dst;

    CHECK(cpu_dst.IsIdentical(npp_res));

    cpu_dst.Sub(constVal, cpu_dst, 1);
    npp_dst.Sub(constVal, npp_dst, 1, nppCtx);
...
TEST_CASE("8uC3", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image8uC3::GetStreamContext();
...
TEST_CASE("8uC4", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image8uC4::GetStreamContext();
...
TEST_CASE("8uC4A", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image8uC4::GetStreamContext();
    ...
    cpu_src1A.Sub(constVal, cpu_dstA);
    npp_src1.SubA(constVal.XYZ(), npp_dst, 0, nppCtx);

    npp_res << npp_dst;

    CHECK(cpu_dst.IsIdentical(npp_res));

    cpu_src1A.Sub(constVal, cpu_dstA, 2);
    npp_src1.SubA(constVal.XYZ(), npp_dst, 2, nppCtx);

    npp_res << npp_dst;

    CHECK(cpu_dst.IsIdentical(npp_res));

    cpu_dstA.Sub(constVal, cpu_dstA, 1);
    npp_dst.SubA(constVal.XYZ(), npp_dst, 1, nppCtx);
...
TEST_CASE("16uC1", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image16uC1::GetStreamContext();
...
TEST_CASE("16uC3", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image16uC3::GetStreamContext();
...
TEST_CASE("16uC4", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image16uC4::GetStreamContext();
...
TEST_CASE("16uC4A", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image16uC4::GetStreamContext();
...
TEST_CASE("32sC1", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image32sC1::GetStreamContext();
...
TEST_CASE("32sC3", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image32sC3::GetStreamContext();
...
TEST_CASE("16fC1", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image16fC1::GetStreamContext();
    ...
    cpu_src1.Sub(constVal, cpu_dst);
    npp_src1.Sub(constVal, npp_dst, nppCtx);

    npp_res << npp_dst;

    CHECK(cpu_dst.IsIdentical(npp_res));

    cpu_dst.Sub(constVal, cpu_dst);
    npp_dst.Sub(constVal, npp_dst, nppCtx);
...
TEST_CASE("16fC3", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image16fC3::GetStreamContext();
...
TEST_CASE("16fC4", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image16fC4::GetStreamContext();
...
TEST_CASE("32fC1", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image32fC1::GetStreamContext();
...
TEST_CASE("32fC3", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image32fC3::GetStreamContext();
...
TEST_CASE("32fC4", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image32fC4::GetStreamContext();
...
TEST_CASE("32fC4A", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image32fC4::GetStreamContext();
    ...
    cpu_src1A.Sub(constVal, cpu_dstA);
    npp_src1.SubA(constVal.XYZ(), npp_dst, nppCtx);

    npp_res << npp_dst;

    CHECK(cpu_dst.IsIdentical(npp_res));

    cpu_dstA.Sub(constVal, cpu_dstA);
    npp_dst.SubA(constVal.XYZ(), npp_dst, nppCtx);
...
TEST_CASE("16scC1", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image16scC1::GetStreamContext();
...
TEST_CASE("16scC3", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image16scC3::GetStreamContext();
...
TEST_CASE("16scC4", "[NPP.Arithmetic.SubC]")
{
    const uint seed         = Catch::getSeed();
    NppStreamContext nppCtx = nv::Image16scC4::GetStreamContext();
