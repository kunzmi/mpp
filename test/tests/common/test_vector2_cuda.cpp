...
template <typename T> Vector2<T> GetRandomValue(std::default_random_engine & /*aEngine*/)
{
    return Vector2<T>();
}
template <typename T>
Vector2<T> GetRandomValue(std::default_random_engine &aEngine)
    requires NativeFloatingPoint<T>
{
    ...
    return Vector2<T>(uniform_dist(aEngine), uniform_dist(aEngine));
}
template <typename T>
Vector2<T> GetRandomValue(std::default_random_engine &aEngine)
    ...
    requires ComplexIntegral<T>
{
    ...
    return Vector2<T>(T(uniform_dist(aEngine), uniform_dist(aEngine)), T(uniform_dist(aEngine), uniform_dist(aEngine)));
}
template <typename T>
Vector2<T> GetRandomValue(std::default_random_engine &aEngine)
    requires NativeIntegral<T> && ByteSizeType<T>
{
    ...
    return Vector2<T>(static_cast<T>(uniform_dist(aEngine)), static_cast<T>(uniform_dist(aEngine)));
}
template <> Vector2<BFloat16> GetRandomValue<BFloat16>(std::default_random_engine &aEngine)
{
    std::uniform_real_distribution<float> uniform_dist(-10000.0_bf, 10000.0_bf);
    return Vector2<BFloat16>(BFloat16(uniform_dist(aEngine)), BFloat16(uniform_dist(aEngine)));
}
template <> Vector2<HalfFp16> GetRandomValue<HalfFp16>(std::default_random_engine &aEngine)
{
    std::uniform_real_distribution<float> uniform_dist(-10, 10);
    return Vector2<HalfFp16>(HalfFp16(uniform_dist(aEngine)), HalfFp16(uniform_dist(aEngine)));
}
template <typename T>
Vector2<T> GetRandomValue(std::default_random_engine &aEngine)
...
template <> Pixel16bfcC2 GetRandomValue<c_BFloat16>(std::default_random_engine &aEngine)
{
    std::uniform_real_distribution<float> uniform_dist(-10000.0_bf, 10000.0_bf);
    return Pixel16bfcC2(
...
template <> Pixel16fcC2 GetRandomValue<c_HalfFp16>(std::default_random_engine &aEngine)
{
    std::uniform_real_distribution<float> uniform_dist(-10, 10);
    return Pixel16fcC2(
...
template <typename T>
void fillData(std::vector<Vector2<T>> &aDataIn, std::vector<Vector2<T>> &aDataOut, std::vector<Pixel8uC2> &aComp,
...
TEST_CASE("Pixel64fC2 CUDA", "[Common]")
{
    cudaSafeCall(cudaSetDevice(DEFAULT_CUDA_DEVICE_ID));

    std::vector<Pixel64fC2> dataIn(sizeIn);
    std::vector<Pixel64fC2> dataOut(sizeOut, Pixel64fC2(0));
    std::vector<Pixel64fC2> dataOutGPU(sizeOut);
    std::vector<Pixel8uC2> dataComp(sizeComp);
    std::vector<Pixel8uC2> dataCompGPU(sizeComp);
    std::vector<double> epsilon(sizeOut, 0.0);

    fillData(dataIn, dataOut, dataComp, epsilon);

    DevVar<Pixel64fC2> d_dataIn(sizeIn);
    DevVar<Pixel64fC2> d_dataOut(sizeOut);
    DevVar<Pixel8uC2> d_dataComp(sizeComp);

    d_dataIn << dataIn;
    d_dataOut.Memset(0);

    runtest_vector2_kernel(d_dataIn.Pointer(), d_dataOut.Pointer(), d_dataComp.Pointer());
    ...
    for (size_t i = 0; i < dataOut.size(); i++)
    {
        if (epsilon[i] != 0.0)
        {
            CHECK(Pixel64fC2::EqEps(dataOut[i], dataOutGPU[i], epsilon[i]));
        }
        else
        {
...
TEST_CASE("Pixel32fC2 CUDA", "[Common]")
{
    cudaSafeCall(cudaSetDevice(DEFAULT_CUDA_DEVICE_ID));

    std::vector<Pixel32fC2> dataIn(sizeIn);
    std::vector<Pixel32fC2> dataOut(sizeOut, Pixel32fC2(0));
    std::vector<Pixel32fC2> dataOutGPU(sizeOut);
    std::vector<Pixel8uC2> dataComp(sizeComp);
    std::vector<Pixel8uC2> dataCompGPU(sizeComp);
    ...
    DevVar<Pixel32fC2> d_dataIn(sizeIn);
    DevVar<Pixel32fC2> d_dataOut(sizeOut);
    DevVar<Pixel8uC2> d_dataComp(sizeComp);
    ...
    for (size_t i = 0; i < dataOut.size(); i++)
    {
        if (epsilon[i] != 0.0f)
        {
            CHECK(Pixel32fC2::EqEps(dataOut[i], dataOutGPU[i], epsilon[i]));
        }
        else
        {
            CHECK(dataOut[i] == dataOutGPU[i]);
...
TEST_CASE("Pixel16bfC2 CUDA", "[Common]")
{
    cudaSafeCall(cudaSetDevice(DEFAULT_CUDA_DEVICE_ID));

    std::vector<Pixel16bfC2> dataIn(sizeIn);
    std::vector<Pixel16bfC2> dataOut(sizeOut, Pixel16bfC2(0.0_bf));
    std::vector<Pixel16bfC2> dataOutGPU(sizeOut);
    std::vector<Pixel8uC2> dataComp(sizeComp);
    std::vector<Pixel8uC2> dataCompGPU(sizeComp);
    ...
    DevVar<Pixel16bfC2> d_dataIn(sizeIn);
    DevVar<Pixel16bfC2> d_dataOut(sizeOut);
    DevVar<Pixel8uC2> d_dataComp(sizeComp);
    ...
    for (size_t i = 0; i < dataOut.size(); i++)
    {
        if (epsilon[i] != 0.0_hf)
        {
            CHECK(Pixel16bfC2::EqEps(dataOut[i], dataOutGPU[i], epsilon[i]));
        }
        else
        {
            CHECK(dataOut[i] == dataOutGPU[i]);
...
TEST_CASE("Pixel16fC2 CUDA", "[Common]")
{
    cudaSafeCall(cudaSetDevice(DEFAULT_CUDA_DEVICE_ID));

    std::vector<Pixel16fC2> dataIn(sizeIn);
    std::vector<Pixel16fC2> dataOut(sizeOut, Pixel16fC2(0.0_hf));
    std::vector<Pixel16fC2> dataOutGPU(sizeOut);
    std::vector<Pixel8uC2> dataComp(sizeComp);
    std::vector<Pixel8uC2> dataCompGPU(sizeComp);
    ...
    DevVar<Pixel16fC2> d_dataIn(sizeIn);
    DevVar<Pixel16fC2> d_dataOut(sizeOut);
    DevVar<Pixel8uC2> d_dataComp(sizeComp);
    ...
    for (size_t i = 0; i < dataOut.size(); i++)
    {
        if (epsilon[i] != 0.0_hf)
        {
            CHECK(Pixel16fC2::EqEps(dataOut[i], dataOutGPU[i], epsilon[i]));
        }
        else
        {
            CHECK(dataOut[i] == dataOutGPU[i]);
...
TEST_CASE("Pixel32sC2 CUDA", "[Common]")
{
    cudaSafeCall(cudaSetDevice(DEFAULT_CUDA_DEVICE_ID));

    std::vector<Pixel32sC2> dataIn(sizeIn);
    std::vector<Pixel32sC2> dataOut(sizeOut, Pixel32sC2(0));
    std::vector<Pixel32sC2> dataOutGPU(sizeOut);
    std::vector<Pixel8uC2> dataComp(sizeComp);
    std::vector<Pixel8uC2> dataCompGPU(sizeComp);
    ...
    DevVar<Pixel32sC2> d_dataIn(sizeIn);
    DevVar<Pixel32sC2> d_dataOut(sizeOut);
    DevVar<Pixel8uC2> d_dataComp(sizeComp);
...
TEST_CASE("Pixel32uC2 CUDA", "[Common]")
{
    cudaSafeCall(cudaSetDevice(DEFAULT_CUDA_DEVICE_ID));

    std::vector<Pixel32uC2> dataIn(sizeIn);
    std::vector<Pixel32uC2> dataOut(sizeOut, Pixel32uC2(0));
    std::vector<Pixel32uC2> dataOutGPU(sizeOut);
    std::vector<Pixel8uC2> dataComp(sizeComp);
    std::vector<Pixel8uC2> dataCompGPU(sizeComp);
    ...
    DevVar<Pixel32uC2> d_dataIn(sizeIn);
    DevVar<Pixel32uC2> d_dataOut(sizeOut);
    DevVar<Pixel8uC2> d_dataComp(sizeComp);
...
TEST_CASE("Pixel16sC2 CUDA", "[Common]")
{
    cudaSafeCall(cudaSetDevice(DEFAULT_CUDA_DEVICE_ID));

    std::vector<Pixel16sC2> dataIn(sizeIn);
    std::vector<Pixel16sC2> dataOut(sizeOut, Pixel16sC2(0));
    std::vector<Pixel16sC2> dataOutGPU(sizeOut);
    std::vector<Pixel8uC2> dataComp(sizeComp);
    std::vector<Pixel8uC2> dataCompGPU(sizeComp);
    ...
    DevVar<Pixel16sC2> d_dataIn(sizeIn);
    DevVar<Pixel16sC2> d_dataOut(sizeOut);
    DevVar<Pixel8uC2> d_dataComp(sizeComp);
    ...
    if constexpr (EnableSIMD<short>)
    {
        ...
        for (size_t i = 0; i < dataOut.size(); i++)
        {
            ...
            {
                if (dataOutGPU[i][static_cast<Axis2D>(channel)] == numeric_limits<short>::lowest() ||
                    dataOutGPU[i][static_cast<Axis2D>(channel)] == numeric_limits<short>::max())
                {
                    dataOut[i][static_cast<Axis2D>(channel)] = dataOutGPU[i][static_cast<Axis2D>(channel)];
                    counter++;
                }
            }
        }
...
TEST_CASE("Pixel16uC2 CUDA", "[Common]")
{
    cudaSafeCall(cudaSetDevice(DEFAULT_CUDA_DEVICE_ID));

    std::vector<Pixel16uC2> dataIn(sizeIn);
    std::vector<Pixel16uC2> dataOut(sizeOut, Pixel16uC2(0));
    std::vector<Pixel16uC2> dataOutGPU(sizeOut);
    std::vector<Pixel8uC2> dataComp(sizeComp);
    std::vector<Pixel8uC2> dataCompGPU(sizeComp);
    ...
    DevVar<Pixel16uC2> d_dataIn(sizeIn);
    DevVar<Pixel16uC2> d_dataOut(sizeOut);
    DevVar<Pixel8uC2> d_dataComp(sizeComp);
    ...
    if constexpr (EnableSIMD<ushort>)
    {
        ...
        for (size_t i = 0; i < dataOut.size(); i++)
        {
            for (int channel = 0; channel < 2; channel++)
            {
                if (dataOutGPU[i][static_cast<Axis2D>(channel)] == numeric_limits<ushort>::lowest() ||
                    dataOutGPU[i][static_cast<Axis2D>(channel)] == numeric_limits<ushort>::max())
                {
                    dataOut[i][static_cast<Axis2D>(channel)] = dataOutGPU[i][static_cast<Axis2D>(channel)];
                    counter++;
                }
...
TEST_CASE("Pixel8sC2 CUDA", "[Common]")
{
    cudaSafeCall(cudaSetDevice(DEFAULT_CUDA_DEVICE_ID));

    std::vector<Pixel8sC2> dataIn(sizeIn);
    std::vector<Pixel8sC2> dataOut(sizeOut, Pixel8sC2(0));
    std::vector<Pixel8sC2> dataOutGPU(sizeOut);
    std::vector<Pixel8uC2> dataComp(sizeComp);
    std::vector<Pixel8uC2> dataCompGPU(sizeComp);
    ...
    DevVar<Pixel8sC2> d_dataIn(sizeIn);
    DevVar<Pixel8sC2> d_dataOut(sizeOut);
    DevVar<Pixel8uC2> d_dataComp(sizeComp);
    ...
    if constexpr (EnableSIMD<sbyte>)
    {
        ...
        for (size_t i = 0; i < dataOut.size(); i++)
        {
            for (int channel = 0; channel < 2; channel++)
            {
                if (dataOutGPU[i][static_cast<Axis2D>(channel)] == numeric_limits<sbyte>::lowest() ||
                    dataOutGPU[i][static_cast<Axis2D>(channel)] == numeric_limits<sbyte>::max())
                {
                    dataOut[i][static_cast<Axis2D>(channel)] = dataOutGPU[i][static_cast<Axis2D>(channel)];
                    counter++;
                }
...
TEST_CASE("Pixel8uC2 CUDA", "[Common]")
{
    cudaSafeCall(cudaSetDevice(DEFAULT_CUDA_DEVICE_ID));

    std::vector<Pixel8uC2> dataIn(sizeIn);
    std::vector<Pixel8uC2> dataOut(sizeOut, Pixel8uC2(0));
    std::vector<Pixel8uC2> dataOutGPU(sizeOut);
    std::vector<Pixel8uC2> dataComp(sizeComp);
    std::vector<Pixel8uC2> dataCompGPU(sizeComp);
    ...
    DevVar<Pixel8uC2> d_dataIn(sizeIn);
    DevVar<Pixel8uC2> d_dataOut(sizeOut);
    DevVar<Pixel8uC2> d_dataComp(sizeComp);
    ...
    if constexpr (EnableSIMD<byte>)
    {
        ...
        for (size_t i = 0; i < dataOut.size(); i++)
        {
            for (int channel = 0; channel < 2; channel++)
            {
                if (dataOutGPU[i][static_cast<Axis2D>(channel)] == numeric_limits<byte>::lowest() ||
                    dataOutGPU[i][static_cast<Axis2D>(channel)] == numeric_limits<byte>::max())
                {
                    dataOut[i][static_cast<Axis2D>(channel)] = dataOutGPU[i][static_cast<Axis2D>(channel)];
                    counter++;
                }
...
TEST_CASE("Pixel64fcC2 CUDA", "[Common]")
{
    cudaSafeCall(cudaSetDevice(DEFAULT_CUDA_DEVICE_ID));

    std::vector<Pixel64fcC2> dataIn(sizeIn);
    std::vector<Pixel64fcC2> dataOut(sizeOut, Pixel64fcC2(0.0_i));
    std::vector<Pixel64fcC2> dataOutGPU(sizeOut);
    std::vector<Pixel8uC2> dataComp(sizeComp);
    std::vector<Pixel8uC2> dataCompGPU(sizeComp);
    std::vector<double> epsilon(sizeOut, 0.0);
    ...
    DevVar<Pixel64fcC2> d_dataIn(sizeIn);
    DevVar<Pixel64fcC2> d_dataOut(sizeOut);
    DevVar<Pixel8uC2> d_dataComp(sizeComp);
    ...
    for (size_t i = 0; i < dataOut.size(); i++)
    {
        if (epsilon[i] != 0.0)
        {
            CHECK(Pixel64fcC2::EqEps(dataOut[i], dataOutGPU[i], epsilon[i]));
        }
        else
        {
            CHECK(dataOut[i] == dataOutGPU[i]);
...
TEST_CASE("Pixel32fcC2 CUDA", "[Common]")
{
    cudaSafeCall(cudaSetDevice(DEFAULT_CUDA_DEVICE_ID));

    std::vector<Pixel32fcC2> dataIn(sizeIn);
    std::vector<Pixel32fcC2> dataOut(sizeOut, Pixel32fcC2(0.0_i));
    std::vector<Pixel32fcC2> dataOutGPU(sizeOut);
    std::vector<Pixel8uC2> dataComp(sizeComp);
    std::vector<Pixel8uC2> dataCompGPU(sizeComp);
    std::vector<float> epsilon(sizeOut, 0.0f);
    ...
    DevVar<Pixel32fcC2> d_dataIn(sizeIn);
    DevVar<Pixel32fcC2> d_dataOut(sizeOut);
    DevVar<Pixel8uC2> d_dataComp(sizeComp);
    ...
    for (size_t i = 0; i < dataOut.size(); i++)
    {
        if (epsilon[i] != 0.0f)
        {
            CHECK(Pixel32fcC2::EqEps(dataOut[i], dataOutGPU[i], epsilon[i]));
        }
        else
        {
            CHECK(dataOut[i] == dataOutGPU[i]);
...
TEST_CASE("Pixel16bfcC2 CUDA", "[Common]")
{
    cudaSafeCall(cudaSetDevice(DEFAULT_CUDA_DEVICE_ID));

    std::vector<Pixel16bfcC2> dataIn(sizeIn);
    std::vector<Pixel16bfcC2> dataOut(sizeOut, Pixel16bfcC2(0.0_i));
    std::vector<Pixel16bfcC2> dataOutGPU(sizeOut);
    std::vector<Pixel8uC2> dataComp(sizeComp);
    std::vector<Pixel8uC2> dataCompGPU(sizeComp);
    std::vector<BFloat16> epsilon(sizeOut, 0.0_bf);
    ...
    DevVar<Pixel16bfcC2> d_dataIn(sizeIn);
    DevVar<Pixel16bfcC2> d_dataOut(sizeOut);
    DevVar<Pixel8uC2> d_dataComp(sizeComp);
    ...
    for (size_t i = 0; i < dataOut.size(); i++)
    {
        if (epsilon[i] != 0.0_hf)
        {
            CHECK(Pixel16bfcC2::EqEps(dataOut[i], dataOutGPU[i], epsilon[i]));
        }
        else
        {
            CHECK(dataOut[i] == dataOutGPU[i]);
...
TEST_CASE("Pixel16fcC2 CUDA", "[Common]")
{
    cudaSafeCall(cudaSetDevice(DEFAULT_CUDA_DEVICE_ID));

    std::vector<Pixel16fcC2> dataIn(sizeIn);
    std::vector<Pixel16fcC2> dataOut(sizeOut, Pixel16fcC2(0.0_i));
    std::vector<Pixel16fcC2> dataOutGPU(sizeOut);
    std::vector<Pixel8uC2> dataComp(sizeComp);
    std::vector<Pixel8uC2> dataCompGPU(sizeComp);
    std::vector<HalfFp16> epsilon(sizeOut, 0.0_hf);
    ...
    DevVar<Pixel16fcC2> d_dataIn(sizeIn);
    DevVar<Pixel16fcC2> d_dataOut(sizeOut);
    DevVar<Pixel8uC2> d_dataComp(sizeComp);
    ...
    for (size_t i = 0; i < dataOut.size(); i++)
    {
        if (epsilon[i] != 0.0_hf)
        {
            CHECK(Pixel16fcC2::EqEps(dataOut[i], dataOutGPU[i], epsilon[i]));
        }
        else
        {
            CHECK(dataOut[i] == dataOutGPU[i]);
...
TEST_CASE("Pixel32scC2 CUDA", "[Common]")
{
    cudaSafeCall(cudaSetDevice(DEFAULT_CUDA_DEVICE_ID));

    std::vector<Pixel32scC2> dataIn(sizeIn);
    std::vector<Pixel32scC2> dataOut(sizeOut, Pixel32scC2(0));
    std::vector<Pixel32scC2> dataOutGPU(sizeOut);
    std::vector<Pixel8uC2> dataComp(sizeComp);
    std::vector<Pixel8uC2> dataCompGPU(sizeComp);
    std::vector<int> epsilon(sizeOut, 0);
    ...
    DevVar<Pixel32scC2> d_dataIn(sizeIn);
    DevVar<Pixel32scC2> d_dataOut(sizeOut);
    DevVar<Pixel8uC2> d_dataComp(sizeComp);
...
TEST_CASE("Pixel16scC2 CUDA", "[Common]")
{
    cudaSafeCall(cudaSetDevice(DEFAULT_CUDA_DEVICE_ID));

    std::vector<Pixel16scC2> dataIn(sizeIn);
    std::vector<Pixel16scC2> dataOut(sizeOut, Pixel16scC2(0));
    std::vector<Pixel16scC2> dataOutGPU(sizeOut);
    std::vector<Pixel8uC2> dataComp(sizeComp);
    std::vector<Pixel8uC2> dataCompGPU(sizeComp);
    std::vector<short> epsilon(sizeOut, 0);
    ...
    DevVar<Pixel16scC2> d_dataIn(sizeIn);
    DevVar<Pixel16scC2> d_dataOut(sizeOut);
    DevVar<Pixel8uC2> d_dataComp(sizeComp);
    ...
    if constexpr (EnableSIMD<short>)
    {
        ...
        for (size_t i = 0; i < dataOut.size(); i++)
        {
            for (int channel = 0; channel < 2; channel++)
            {
                if (dataOutGPU[i][static_cast<Axis2D>(channel)].real == numeric_limits<short>::lowest() ||
                    dataOutGPU[i][static_cast<Axis2D>(channel)].real == numeric_limits<short>::max())
                {
                    dataOut[i][static_cast<Axis2D>(channel)].real = dataOutGPU[i][static_cast<Axis2D>(channel)].real;
                    counter++;
                }
                if (dataOutGPU[i][static_cast<Axis2D>(channel)].imag == numeric_limits<short>::lowest() ||
                    dataOutGPU[i][static_cast<Axis2D>(channel)].imag == numeric_limits<short>::max())
                {
                    dataOut[i][static_cast<Axis2D>(channel)].imag = dataOutGPU[i][static_cast<Axis2D>(channel)].imag;
                    counter++;
                }
            }
        }
