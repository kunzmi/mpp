...
// declare the CUDA intrinsic used later, otherwise clang-tidy gets crazy...
...
template <typename T, std::size_t size> struct alignas(size * sizeof(T)) Tupel
{
    ...
    /// </summary>
    bool DEVICE_CODE AreAllBytesFalse()
        requires std::same_as<byte, T> && //
                 (size == 1)
    {
        return value[0] == static_cast<byte>(0);
    }
    ...
    /// </summary>
    bool DEVICE_CODE AreAllBytesTrue()
        requires std::same_as<byte, T> && //
                 (size == 1)
    {
        return value[0] != static_cast<byte>(0);
                 ...
                 (size == 2)
    {
        ...
        return value[0] != static_cast<byte>(0) && value[1] != static_cast<byte>(0);
    }
    ...
    {
        return value[0] != static_cast<byte>(0) && value[1] != static_cast<byte>(0) &&
               value[2] != static_cast<byte>(0) && value[2] != static_cast<byte>(0);
    }
    ...
    /// </summary>
    bool DEVICE_ONLY_CODE AreAllBytesTrue()
        requires std::same_as<byte, T> && //
                 (size == 4) &&           //
                 CUDA_ONLY<T> && DeviceCode<T>
    {
        uint &ui = *reinterpret_cast<uint *>(this);
