...
{
/// <summary>
/// Computes an output pixel from 411 downsampled src arrays -&gt; dst pixel<para/>
/// First channel is Luma channel at full resolution, second and third channel are chroma with 1/4 width and full
...
struct Src411Functor : public ImageFunctor<false>
{
...
#pragma region Constructors
    Src411Functor(const Src1Plane *aSrc1, size_t aSrcPitch1, const Src2Plane *aSrc2, size_t aSrcPitch2,
    ...
    Src411Functor(const Src1Plane *aSrc1, size_t aSrcPitch1, const Src1Plane *aSrc21, size_t aSrcPitch21,
...
#pragma endregion
  ...
  private:
    ...
    {
        int iChromaX = aLumaX / 4; // integer division with floor()
        int iChromaY = aLumaY;
        ...
        {
            // the pixel index in the 4x1 YUV pixel block:
            int offsetX = aLumaX & 3;

            if constexpr ((chromaSubsamplePos == ChromaSubsamplePos::Undefined ||
                           chromaSubsamplePos == ChromaSubsamplePos::Center))
            {
                aChromaX += 0.25f * static_cast<float>(offsetX) - 0.375f;
            }
            ...
            {
                aChromaX += 0.25f * static_cast<float>(offsetX);
            }
        }
    }
};
