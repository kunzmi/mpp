...
struct InplaceSrcSrcFunctor
{
    const SrcType *__restrict__ Src1;
    size_t SrcPitch1;

    const SrcType *__restrict__ Src2;
    size_t SrcPitch2;
    ...
    InplaceSrcSrcFunctor(SrcType *aSrc1, size_t aSrcPitch1, SrcType *aSrc2, size_t aSrcPitch2, operation aOp)
        ...
        requires std::same_as<ComputeType, ResultType>
    {
        ...
        const SrcType *pixelSrc2 = gotoPtr(Src2, SrcPitch2, aPixelX, aPixelY);
        Op(ComputeType(*pixelSrc1), ComputeType(*pixelSrc2), aDst);
    }
        ...
        requires std::same_as<ComputeType, ResultType> && (tupelSize > 1)
    {
        ...
        Tupel<SrcType, tupelSize> tupelSrc2 = Tupel<SrcType, tupelSize>::Load(pixelSrc2);

#pragma unroll
        for (size_t i = 0; i < tupelSize; i++)
        {
            Op(ComputeType(tupelSrc1.value[i]), ComputeType(tupelSrc2.value[i]), aDst.value[i]);
        }
    }
...
#pragma unroll
        for (size_t i = 0; i < tupelSize; i++)
        {
            ...
            Op(ComputeType(tupelSrc1.value[i]), ComputeType(tupelSrc2.value[i]), temp);
            round(temp);
            // ResultType constructor will clamp temp to value range of ResultType
