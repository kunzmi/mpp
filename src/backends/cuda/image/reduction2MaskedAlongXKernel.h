...
__global__ void reduction2MaskedAlongXKernel(const byte *__restrict__ aMask, size_t aPitchMask,
                                             DstT1 *__restrict__ aDst1, DstT2 *__restrict__ aDst2, Size2D aSize,
                                             ThreadSplit<WarpAlignmentInBytes, TupelSize> aSplit, funcType aFunctor)
{
    ...
    else
    {
             ...
             pixelXWarp0 += warpSize)
        {
            ...
            aFunctor(pixelX, pixelY, result1, result2, maskTupel);
        }
...
void InvokeReduction2MaskedAlongXKernel(const dim3 &aBlockSize, uint aSharedMemory, int aWarpSize, cudaStream_t aStream,
                                        const byte *aMask, size_t aPitchMask, const SrcT *aSrc, DstT1 *aDst1,
                                        DstT2 *aDst2, const Size2D &aSize, const funcType &aFunctor)
{
    ThreadSplit<WarpAlignmentInBytes, TupelSize> ts(aSrc, aSize.x, aWarpSize);

    dim3 blocksPerGrid(1, DIV_UP(aSize.y, aBlockSize.y), 1);

    reduction2MaskedAlongXKernel<WarpAlignmentInBytes, TupelSize, DstT1, DstT2, funcType, reductionOp1, reductionOp2,
                                 NeutralValue1, NeutralValue2>
        <<<blocksPerGrid, aBlockSize, aSharedMemory, aStream>>>(aMask, aPitchMask, aDst1, aDst2, aSize, ts, aFunctor);
...
template <typename SrcT, typename DstT1, typename DstT2, size_t TupelSize, typename funcType, typename reductionOp1,
          typename reductionOp2, ReductionInitValue NeutralValue1, ReductionInitValue NeutralValue2>
void InvokeReduction2MaskedAlongXKernelDefault(const Pixel8uC1 *aMask, size_t aPitchMask, const SrcT *aSrc,
                                               DstT1 *aDst1, DstT2 *aDst2, const Size2D &aSize,
                                               const mpp::cuda::StreamCtx &aStreamCtx, const funcType &aFunc)
{
    if (aStreamCtx.ComputeCapabilityMajor < INT_MAX)
    {
        ...
        InvokeReduction2MaskedAlongXKernel<SrcT, DstT1, DstT2, TupelSize, WarpAlignmentInBytes, funcType, reductionOp1,
            ...
            aPitchMask, aSrc, aDst1, aDst2, aSize, aFunc);
    }
    else
    {
        throw CUDAUNSUPPORTED(reduction2MaskedAlongXKernel,
