...
{
/// <summary>
/// Applies a separable filter to each pixel in an image with border control.<para/>
...
__global__ void separableFilterKernel(BorderControlT aSrcWithBC, DstT *__restrict__ aDst, size_t aPitchDst,
                                      const FilterT *__restrict__ aFilter, FilterT aScalingValueInv, int aFilterSize,
...
template <typename ComputeT, typename DstT, size_t TupelSize, int WarpAlignmentInBytes, int blockHeight,
          RoundingMode roundingMode, typename BorderControlT, typename FilterT, bool needsScaling>
void InvokeSeparableFilterKernel(const dim3 &aBlockSize, uint aSharedMemory, int aWarpSize, cudaStream_t aStream,
                                 const BorderControlT &aSrcWithBC, DstT *aDst, size_t aPitchDst, const FilterT *aFilter,
                                 FilterT aScalingValueInv, int aFilterSize, int aFilterCenter, const Size2D &aSize)
{
    ...
    separableFilterKernel<WarpAlignmentInBytes, TupelSize, ComputeT, DstT, blockHeight, roundingMode, BorderControlT,
                          FilterT, needsScaling><<<blocksPerGrid, aBlockSize, aSharedMemory, aStream>>>(
        aSrcWithBC, aDst, aPitchDst, aFilter, aScalingValueInv, aFilterSize, aFilterCenter, aSize, ts);
...
void InvokeSeparableFilterKernelDefault(const BorderControlT &aSrcWithBC, DstT *aDst, size_t aPitchDst,
                                        ...
                                        int aFilterCenter, const Size2D &aSize, const mpp::cuda::StreamCtx &aStreamCtx)
{
    if (aStreamCtx.ComputeCapabilityMajor < INT_MAX)
    {
        ...
        {
            InvokeSeparableFilterKernel<ComputeT, DstT, TupelSize, WarpAlignmentInBytes, blockHeight, roundingMode,
        ...
        else if (blockHeight > 1)
        {
            ...
            InvokeSeparableFilterKernel<ComputeT, DstT, TupelSize, WarpAlignmentInBytes, 1, roundingMode,
                                        BorderControlT, FilterT, needsScaling>(
                BlockSize, SharedMemory2, aStreamCtx.WarpSize, aStreamCtx.Stream, aSrcWithBC, aDst, aPitchDst, aFilter,
                aScalingValueInv, aFilterSize, aFilterCenter, aSize);
        }
        else
        {
    ...
    {
        throw CUDAUNSUPPORTED(separableFilterKernel,
