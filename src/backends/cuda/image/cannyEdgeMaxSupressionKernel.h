...
__device__ DstT processOnePixel(int pixelX, int pixelY, BorderControlT &aSrcWithBC,
                                const Pixel32fC1 *__restrict__ aSrcAngle, size_t aPitchSrcAngle, SrcT aLowThreshold,
                                SrcT aHighThreshold)
{
...
/// </summary>
template <int WarpAlignmentInBytes, int TupelSize, class SrcT, class DstT, typename BorderControlT>
__global__ void cannyEdgeMaxSupressionKernel(BorderControlT aSrcWithBC, const Pixel32fC1 *__restrict__ aSrcAngle,
                                             size_t aPitchSrcAngle, DstT *__restrict__ aDst, size_t aPitchDst,
                                             SrcT aLowThreshold, SrcT aHighThreshold, Size2D aSize,
...
#pragma unroll
            ...
            {
                res.value[t] = processOnePixel<SrcT, DstT, BorderControlT>(
                    pixelX + t, pixelY, aSrcWithBC, aSrcAngle, aPitchSrcAngle, aLowThreshold, aHighThreshold);
            }
    ...
    *pixelOut      = processOnePixel<SrcT, DstT, BorderControlT>(pixelX, pixelY, aSrcWithBC, aSrcAngle, aPitchSrcAngle,
                                                                 aLowThreshold, aHighThreshold);

    return;
}

template <typename DstT, size_t TupelSize, int WarpAlignmentInBytes, class SrcT, typename BorderControlT>
void InvokeCannyEdgeMaxSupressionKernel(const dim3 &aBlockSize, uint aSharedMemory, int aWarpSize, cudaStream_t aStream,
                                        ...
                                        SrcT aHighThreshold, const Size2D &aSize)
{
    ...
    cannyEdgeMaxSupressionKernel<WarpAlignmentInBytes, TupelSize, SrcT, DstT, BorderControlT>
        <<<blocksPerGrid, aBlockSize, aSharedMemory, aStream>>>(aSrcWithBC, aSrcAngle, aPitchSrcAngle, aDst, aPitchDst,
                                                                aLowThreshold, aHighThreshold, aSize, ts);
...
template <typename DstT, size_t TupelSize, class SrcT, typename BorderControlT>
void InvokeCannyEdgeMaxSupressionKernelDefault(const BorderControlT &aSrcWithBC, const Pixel32fC1 *aSrcAngle,
                                               size_t aPitchSrcAngle, DstT *aDst, size_t aPitchDst, SrcT aLowThreshold,
                                               SrcT aHighThreshold, const Size2D &aSize,
                                               const opp::cuda::StreamCtx &aStreamCtx)
{
    if (aStreamCtx.ComputeCapabilityMajor < INT_MAX)
    {
        ...
        InvokeCannyEdgeMaxSupressionKernel<DstT, TupelSize, WarpAlignmentInBytes, SrcT, BorderControlT>(
            BlockSize, SharedMemory, aStreamCtx.WarpSize, aStreamCtx.Stream, aSrcWithBC, aSrcAngle, aPitchSrcAngle,
            aDst, aPitchDst, aLowThreshold, aHighThreshold, aSize);
    }
    else
    {
        throw CUDAUNSUPPORTED(cannyEdgeMaxSupressionKernel,
