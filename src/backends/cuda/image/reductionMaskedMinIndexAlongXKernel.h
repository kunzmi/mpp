...
__global__ void reductionMaskedMinIdxAlongXKernel(const byte *__restrict__ aMask, size_t aPitchMask, const SrcT *aSrc,
                                                  size_t aPitchSrc, SrcT *__restrict__ aDstMin,
                                                  ...
                                                  Size2D aSize, ThreadSplit<WarpAlignmentInBytes, TupelSize> aSplit)
{
    ...
    else
    {
             ...
             pixelXWarp0 += warpSize)
        {
            ...
            functor(pixelX, pixelY, resultMin, resultMinIdx, maskTupel);
        }
...
void InvokeReductionMaskedMinIdxAlongXKernel(const dim3 &aBlockSize, uint aSharedMemory, int aWarpSize,
                                             ...
                                             same_vector_size_different_type_t<SrcT, int> *aDstMinIdx, Size2D aSize)
{
    ...
    reductionMaskedMinIdxAlongXKernel<WarpAlignmentInBytes, TupelSize>
        <<<blocksPerGrid, aBlockSize, aSharedMemory, aStream>>>(aMask, aPitchMask, aSrc, aPitchSrc, aDstMin, aDstMinIdx,
                                                                aSize, ts);
...
void InvokeReductionMaskedMinIdxAlongXKernelDefault(const Pixel8uC1 *aMask, size_t aPitchMask, const SrcT *aSrc,
                                                    size_t aPitchSrc, SrcT *aDstMin,
                                                    ...
                                                    const Size2D &aSize, const mpp::cuda::StreamCtx &aStreamCtx)
{
    if (aStreamCtx.ComputeCapabilityMajor < INT_MAX)
    {
        ...
        InvokeReductionMaskedMinIdxAlongXKernel<SrcT, TupelSize, WarpAlignmentInBytes>(
            BlockSize, SharedMemory, aStreamCtx.WarpSize, aStreamCtx.Stream, reinterpret_cast<const byte *>(aMask),
            aPitchMask, aSrc, aPitchSrc, aDstMin, aDstMinIdx, aSize);
    }
    else
    {
