...
__global__ void reductionMinMaxIdxAlongXKernel(const SrcT *aSrc, size_t aPitchSrc, SrcT *__restrict__ aDstMin,
                                               SrcT *__restrict__ aDstMax,
                                               ...
                                               Size2D aSize, ThreadSplit<WarpAlignmentInBytes, TupelSize> aSplit)
{
    ...
    else
    {
             ...
             pixelXWarp0 += warpSize)
        {
            ...
            functor(pixelX, pixelY, resultMin, resultMax, resultMinIdx, resultMaxIdx, true);
        }
...
void InvokeReductionMinMaxIdxAlongXKernel(const dim3 &aBlockSize, uint aSharedMemory, int aWarpSize,
                                          cudaStream_t aStream, const SrcT *aSrc, size_t aPitchSrc, SrcT *aDstMin,
                                          SrcT *aDstMax, same_vector_size_different_type_t<SrcT, int> *aDstMinIdx,
                                          same_vector_size_different_type_t<SrcT, int> *aDstMaxIdx, Size2D aSize)
{
    ...
    reductionMinMaxIdxAlongXKernel<WarpAlignmentInBytes, TupelSize>
        <<<blocksPerGrid, aBlockSize, aSharedMemory, aStream>>>(aSrc, aPitchSrc, aDstMin, aDstMax, aDstMinIdx,
                                                                aDstMaxIdx, aSize, ts);
                                                 ...
                                                 const Size2D &aSize, const opp::cuda::StreamCtx &aStreamCtx)
{
    if (aStreamCtx.ComputeCapabilityMajor < INT_MAX)
    {
        ...
        InvokeReductionMinMaxIdxAlongXKernel<SrcT, TupelSize, WarpAlignmentInBytes>(
            BlockSize, SharedMemory, aStreamCtx.WarpSize, aStreamCtx.Stream, aSrc, aPitchSrc, aDstMin, aDstMax,
            aDstMinIdx, aDstMaxIdx, aSize);
    }
    else
    {
        throw CUDAUNSUPPORTED(reductionMinMaxIdxAlongXKernel,
