...
/// Filter coefficients are given by user. <para/>
...
/// </summary>
template <int WarpAlignmentInBytes, int TupelSize, class ComputeT, class DstT, typename FilterT, int kernelWidth,
          int kernelHeight, int blockWidth, int blockHeight, RoundingMode roundingMode, typename BorderControlT>
__global__ void fixedSizeFilterKernel(BorderControlT aSrcWithBC, DstT *__restrict__ aDst, size_t aPitchDst,
...
#pragma unroll
                        for (int kx = 0; kx < kernelWidth; kx++)
                        {
                            ...
                            if (pixelDstY >= 0 && pixelDstY < blockHeight)
                            {
                                ...
                                {
                                    result[pixelDstY][pixelDstX] += srcPixel * filterBuffer[ky * kernelWidth + kx];
                                }
                            }
                        }
                    }
...
template <typename ComputeT, typename DstT, size_t TupelSize, int WarpAlignmentInBytes, typename FilterT,
          int kernelWidth, int kernelHeight, int blockWidth, int blockHeight, RoundingMode roundingMode,
          typename BorderControlT>
void InvokeFixedSizeFilterKernel(const dim3 &aBlockSize, uint aSharedMemory, int aWarpSize, cudaStream_t aStream,
                                 ...
                                 const Vector2<int> &aFilterCenter, const Size2D &aSize)
{
    ...
    fixedSizeFilterKernel<WarpAlignmentInBytes, TupelSize, ComputeT, DstT, FilterT, kernelWidth, kernelHeight,
                          blockWidth, blockHeight, roundingMode, BorderControlT>
...
template <typename ComputeT, typename DstT, size_t TupelSize, typename FilterT, int kernelWidth, int kernelHeight,
                                        ...
                                        const opp::cuda::StreamCtx &aStreamCtx)
{
    if (aStreamCtx.ComputeCapabilityMajor < INT_MAX)
    {
        ...
        InvokeFixedSizeFilterKernel<ComputeT, DstT, TupelSize, WarpAlignmentInBytes, FilterT, kernelWidth, kernelHeight,
                                    blockWidth, blockHeight, roundingMode, BorderControlT>(
    ...
    {
        throw CUDAUNSUPPORTED(fixedSizeFilterKernel,
