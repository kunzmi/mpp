...
/// non-tupled data, the blockWidth-parameter can be freely set.<para/> The filter is copied to shared memory before any
/// processing.
/// </summary>
template <int WarpAlignmentInBytes, int TupelSize, class DstT, typename FilterT, int blockWidth, int blockHeight,
          typename BorderControlT, typename morphOperation, typename postOp>
__global__ void morphologyKernelSharedMem(BorderControlT aSrcWithBC, DstT *__restrict__ aDst, size_t aPitchDst,
...
#pragma unroll
                        for (int bx = 0; bx < TupelSize; bx++)
                        {
                            ...
                            {
                                aMorph(filterBuffer[filterY * aFilterArea.Size.x + filterX], srcPixel, result[by][bx]);
                            }
                        }
                    }
                }
...
template <int WarpAlignmentInBytes, int TupelSize, class DstT, typename FilterT, int blockWidth, int blockHeight,
          typename BorderControlT, typename morphOperation, typename postOp>
__global__ void morphologyKernel(BorderControlT aSrcWithBC, DstT *__restrict__ aDst, size_t aPitchDst,
                                 const FilterT *__restrict__ aMask, FilterArea aFilterArea, Size2D aSize,
                                 ThreadSplit<WarpAlignmentInBytes, TupelSize> aSplit, morphOperation aMorph,
                                 postOp aPostOp)
{
...
#pragma unroll
                        for (int bx = 0; bx < TupelSize; bx++)
                        {
                            ...
                            if (filterY >= 0 && filterY < aFilterArea.Size.y && filterX >= 0 &&
                                filterX < aFilterArea.Size.x)
                            {
                                aMorph(aMask[filterY * aFilterArea.Size.x + filterX], srcPixel, result[by][bx]);
                            }
                        }
                    }
                }
...
template <typename DstT, size_t TupelSize, int WarpAlignmentInBytes, typename FilterT, int blockWidth, int blockHeight,
          typename BorderControlT, typename morphOperation, typename postOp>
void InvokeMorphologyKernel(const dim3 &aBlockSize, uint aSharedMemory, int aWarpSize, cudaStream_t aStream,
                            const BorderControlT &aSrcWithBC, DstT *aDst, size_t aPitchDst, const FilterT *aMask,
                            const FilterArea &aFilterArea, const Size2D &aSize, morphOperation aMorph, postOp aPostOp)
{
    ...
    if (aSharedMemory == 0)
    {
        morphologyKernel<WarpAlignmentInBytes, TupelSize, DstT, FilterT, blockWidth, blockHeight, BorderControlT,
                         morphOperation, postOp><<<blocksPerGrid, aBlockSize, aSharedMemory, aStream>>>(
            aSrcWithBC, aDst, aPitchDst, aMask, aFilterArea, aSize, ts, aMorph, aPostOp);
    }
    else
    {
        morphologyKernelSharedMem<WarpAlignmentInBytes, TupelSize, DstT, FilterT, blockWidth, blockHeight,
                                  BorderControlT, morphOperation, postOp>
            <<<blocksPerGrid, aBlockSize, aSharedMemory, aStream>>>(aSrcWithBC, aDst, aPitchDst, aMask, aFilterArea,
                                   ...
                                   postOp aPostOp, const opp::cuda::StreamCtx &aStreamCtx)
{
    if (aStreamCtx.ComputeCapabilityMajor < INT_MAX)
    {
        ...
        InvokeMorphologyKernel<DstT, TupelSize, WarpAlignmentInBytes, FilterT, blockWidth, blockHeight, BorderControlT,
                               morphOperation, postOp>(BlockSize, SharedMemory, aStreamCtx.WarpSize, aStreamCtx.Stream,
                                                       aSrcWithBC, aDst, aPitchDst, aMask, aFilterArea, aSize, aMorph,
                                                       aPostOp);
    }
    else
    {
        throw CUDAUNSUPPORTED(morphologyKernel,
