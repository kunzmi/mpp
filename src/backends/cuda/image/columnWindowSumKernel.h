...
__global__ void columnWindowSumKernel(BorderControlT aSrcWithBC, DstT *__restrict__ aDst, size_t aPitchDst,
...
template <typename ComputeT, typename DstT, size_t TupelSize, int WarpAlignmentInBytes, int blockHeight,
          RoundingMode roundingMode, typename BorderControlT, typename FilterT>
void InvokeColumnWindowSumKernel(const dim3 &aBlockSize, uint aSharedMemory, int aWarpSize, cudaStream_t aStream,
                                 ...
                                 int aFilterSize, int aFilterCenter, const Size2D &aSize)
{
    ...
    columnWindowSumKernel<WarpAlignmentInBytes, TupelSize, ComputeT, DstT, blockHeight, roundingMode, BorderControlT,
                          FilterT><<<blocksPerGrid, aBlockSize, aSharedMemory, aStream>>>(
        aSrcWithBC, aDst, aPitchDst, aScalingValue, aFilterSize, aFilterCenter, aSize, ts);
...
void InvokeColumnWindowSumKernelDefault(const BorderControlT &aSrcWithBC, DstT *aDst, size_t aPitchDst,
                                        ...
                                        const mpp::cuda::StreamCtx &aStreamCtx)
{
    if (aStreamCtx.ComputeCapabilityMajor < INT_MAX)
    {
        ...
        InvokeColumnWindowSumKernel<ComputeT, DstT, TupelSize, WarpAlignmentInBytes, blockHeight, roundingMode,
                                    BorderControlT, FilterT>(BlockSize, SharedMemory, aStreamCtx.WarpSize,
                                                             aStreamCtx.Stream, aSrcWithBC, aDst, aPitchDst,
                                                             aScalingValue, aFilterSize, aFilterCenter, aSize);
    }
    else
    {
        throw CUDAUNSUPPORTED(columnWindowSumKernel,
