...
/// with 422 chroma sub-sampling.
/// </summary>
template <class DstT, typename funcType, ChromaSubsamplePos chromaSubsamplePos>
__global__ void forEachPixel422Planar3Kernel(Vector1<remove_vector_t<DstT>> *__restrict__ aDst1, size_t aPitchDst1,
...
void InvokeForEachPixel422Planar3Kernel(const dim3 &aBlockSize, cudaStream_t aStream,
                                        ...
                                        ChromaSubsamplePos chromaSubsamplePos)
{
    ...
    {
        forEachPixel422Planar3Kernel<DstT, funcType, ChromaSubsamplePos::Center>
            <<<blocksPerGrid, aBlockSize, 0, aStream>>>(aDst1, aPitchDst1, aDst2, aPitchDst2, aDst3, aPitchDst3, aSize,
                                                        aFunctor);
    }
    else
    {
        forEachPixel422Planar3Kernel<DstT, funcType, ChromaSubsamplePos::Left>
            <<<blocksPerGrid, aBlockSize, 0, aStream>>>(aDst1, aPitchDst1, aDst2, aPitchDst2, aDst3, aPitchDst3, aSize,
                                                        aFunctor);
    }
...
template <typename DstT, typename funcType>
void InvokeForEachPixel422Planar3KernelDefault(Vector1<remove_vector_t<DstT>> *__restrict__ aDst1, size_t aPitchDst1,
                                               Vector1<remove_vector_t<DstT>> *__restrict__ aDst2, size_t aPitchDst2,
                                               Vector1<remove_vector_t<DstT>> *__restrict__ aDst3, size_t aPitchDst3,
                                               ...
                                               const mpp::cuda::StreamCtx &aStreamCtx, const funcType &aFunc)
{
    if (aStreamCtx.ComputeCapabilityMajor < INT_MAX)
    {
        ...
        InvokeForEachPixel422Planar3Kernel<DstT, funcType>(BlockSize, aStreamCtx.Stream, aDst1, aPitchDst1, aDst2,
                                                           aPitchDst2, aDst3, aPitchDst3, aSize, aFunc,
                                                           chromaSubsamplePos);
    }
    else
    {
        throw CUDAUNSUPPORTED(forEachPixel422Planar3Kernel,
