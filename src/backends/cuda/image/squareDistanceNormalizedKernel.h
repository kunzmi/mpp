...
{
/// <summary>
/// Similar to filterKernel, but adapted slightly for CrossCorrelation/SqrDistance
/// </summary>
...
__global__ void squareDistanceNormalizedKernelSharedMem(BorderControlT aSrcWithBC, DstT *__restrict__ aDst,
                                                        size_t aPitchDst, const SrcT *__restrict__ aTemplate,
                                                        size_t aPitchTemplate, Size2D aTemplateSize, Size2D aSize,
...
#pragma unroll
                        for (int bx = 0; bx < TupelSize; bx++)
                        {
                            ...
                            {
                                ComputeT diff = srcPixel - templateBuffer[tplY * aTemplateSize.x + tplX];
                                result[by][bx] += diff * diff;
                                imgSqr[by][bx] += srcPixel * srcPixel;
                            }
                        }
                    }
                }
...
#pragma unroll
                    ...
                    {
                        ComputeT norm = imgSqr[bl][t] * sumBuffer[0];
                        norm.Sqrt();
                        result[bl][t] /= norm;
...
#pragma unroll
                for (int bc = 0; bc < blockWidth; bc++)
                {
                    if (pixelX + bc < aSize.x)
                    {
                        ...
                        ComputeT norm = imgSqr[bl][bc] * sumBuffer[0];
                        norm.Sqrt();
                        result[bl][bc] /= norm;
...
template <int WarpAlignmentInBytes, int TupelSize, class ComputeT, class DstT, typename SrcT, int blockWidth,
          int blockHeight, RoundingMode roundingMode, typename BorderControlT>
__global__ void squareDistanceNormalizedKernel(BorderControlT aSrcWithBC, DstT *__restrict__ aDst, size_t aPitchDst,
                                               ...
                                               ThreadSplit<WarpAlignmentInBytes, TupelSize> aSplit)
{
    ...
    ComputeT sumSqrTemplate = ComputeT(0);
    __shared__ ComputeT sumBuffer[8]; // maximum block size in y is 4...
...
#pragma unroll
                        for (int bx = 0; bx < TupelSize; bx++)
                        {
                            ...
                            if (tplY >= 0 && tplY < aTemplateSize.y && tplX >= 0 && tplX < aTemplateSize.x)
                            {
                                ...
                                ComputeT diff = srcPixel - ComputeT(tpl);
                                result[by][bx] += diff * diff;
                                imgSqr[by][bx] += srcPixel * srcPixel;
                            }
                        }
...
template <typename ComputeT, typename DstT, size_t TupelSize, int WarpAlignmentInBytes, typename SrcT, int blockWidth,
          int blockHeight, RoundingMode roundingMode, typename BorderControlT>
void InvokeSquareDistanceNormalizedKernel(const dim3 &aBlockSize, uint aSharedMemory, int aWarpSize,
                                          ...
                                          const Size2D &aTemplateSize, const Size2D &aSize)
{
    ...
    if (aSharedMemory == 0)
    {
        squareDistanceNormalizedKernel<WarpAlignmentInBytes, TupelSize, ComputeT, DstT, SrcT, blockWidth, blockHeight,
    ...
    {
        squareDistanceNormalizedKernelSharedMem<WarpAlignmentInBytes, TupelSize, ComputeT, DstT, SrcT, blockWidth,
...
void InvokeSquareDistanceNormalizedKernelDefault(const BorderControlT &aSrcWithBC, DstT *aDst, size_t aPitchDst,
                                                 ...
                                                 const opp::cuda::StreamCtx &aStreamCtx)
{
    if (aStreamCtx.ComputeCapabilityMajor < INT_MAX)
    {
        ...
        InvokeSquareDistanceNormalizedKernel<ComputeT, DstT, TupelSize, WarpAlignmentInBytes, SrcT, blockWidth,
                                             blockHeight, roundingMode, BorderControlT>(
            BlockSize, SharedMemory, aStreamCtx.WarpSize, aStreamCtx.Stream, aSrcWithBC, aDst, aPitchDst, aTemplate,
            aPitchTemplate, aTemplateSize, aSize);
    }
    else
    {
        throw CUDAUNSUPPORTED(squareDistanceKernel,
