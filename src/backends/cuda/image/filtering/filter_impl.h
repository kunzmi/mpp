...
#include <backends/cuda/image/filterKernel.h>
                  ...
                  const Vector2<int> &aOffsetToActualRoi, const Size2D &aSize, const mpp::cuda::StreamCtx &aStreamCtx)
{
    ...
    switch (aBorderType)
    {
        case mpp::BorderType::None:
        {
            ...
            InvokeFilterKernelDefault<ComputeT, DstT, TupelSize, FilterT, pixelBlockSizeX, pixelBlockSizeY,
                                      RoundingMode::NearestTiesToEven, BCType>(bc, aDst, aPitchDst, aFilter,
                                                                               aFilterArea, aSize, aStreamCtx);
        }
        break;
...
#define Instantiate_For(typeSrc, typeDst)                                                                              \
    template void InvokeFilter<typeSrc, typeDst>(                                                                      \
        const typeSrc *aSrc1, size_t aPitchSrc1, typeDst *aDst, size_t aPitchDst,                                      \
        const filtertype_for_t<filter_compute_type_for_t<typeSrc>> *aFilter, const FilterArea &aFilterArea,            \
