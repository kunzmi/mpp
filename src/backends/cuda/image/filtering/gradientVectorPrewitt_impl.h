...
void InvokeGradientVectorPrewitt(const SrcT *aSrc1, size_t aPitchSrc1, DstT *aDstX, size_t aPitchDstX, DstT *aDstY,
        ...
        switch (aMaskSize)
        {
            case MaskSize::Mask_3x3:
            {
                ...
                using FixedFilterKernelXT = // NPP also uses an inverted kernel!
                    FixedInvertedFilterKernel<mpp::FixedFilter::PrewittVert, filterSize, FilterT>;
                using FixedFilterKernelYT = FixedFilterKernel<mpp::FixedFilter::PrewittHoriz, filterSize, FilterT>;
                ...
                switch (aBorderType)
                {
                    ...
                    default:
                        throw INVALIDARGUMENT(
                            aBorderType,
                            aBorderType << " is not a supported border type mode for GradientVectorPrewitt filter.");
                        break;
                }
            }
            break;
            default:
                throw INVALIDARGUMENT(aMaskSize, "Invalid MaskSize for GradientVectorPrewitt filter: " << aMaskSize);
                break;
        }
    }
}
...
#define Instantiate_For(typeSrc, typeDst)                                                                              \
    template void InvokeGradientVectorPrewitt<typeSrc, typeDst>(                                                       \
