...
#include <backends/cuda/image/columnCoefficientFilterKernel.h>
...
void InvokeColumnCoefficientFilter(const SrcT *aSrc1, size_t aPitchSrc1, DstT *aDst, size_t aPitchDst,
    ...
    if constexpr (oppEnablePixelType<DstT> && oppEnableCudaBackend<DstT>)
    {
        ...
        switch (aBorderType)
        {
            case opp::BorderType::None:
            {
                ...
                InvokeColumnCoefficientFilterKernelDefault<ComputeT, DstT, TupelSize, pixelBlockSizeY,
            ...
            default:
                throw INVALIDARGUMENT(aBorderType,
                                      aBorderType << " is not a supported border type mode for column filter.");
                break;
        }
    }
}
...
#define Instantiate_For(typeSrcIsTypeDst, typeFilter)                                                                  \
    template void InvokeColumnCoefficientFilter<typeSrcIsTypeDst, typeSrcIsTypeDst, typeFilter>(                       \
...
ForAllChannelsWithAlpha(8u, float);
...
ForAllChannelsNoAlpha(32fc, float);
