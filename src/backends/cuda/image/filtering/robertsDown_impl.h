...
void InvokeRobertsDown(const SrcT *aSrc1, size_t aPitchSrc1, DstT *aDst, size_t aPitchDst, MaskSize aMaskSize,
    ...
    if constexpr (mppEnablePixelType<DstT> && mppEnableCudaBackend<DstT>)
    {
        ...
        switch (aMaskSize)
        {
            case MaskSize::Mask_3x3:
            {
                ...
                using FixedFilterKernelT = FixedFilterKernel<mpp::FixedFilter::RobertsDown, filterSize, FilterT>;
                ...
                switch (aBorderType)
                {
                    ...
                    default:
                        throw INVALIDARGUMENT(
                            aBorderType, aBorderType << " is not a supported border type mode for RobertsDown filter.");
                        break;
                }
            }
            break;
            default:
                throw INVALIDARGUMENT(aMaskSize, "Invalid MaskSize for RobertsDown filter: " << aMaskSize);
                break;
        }
    }
}
...
#define Instantiate_For(typeSrc, typeDst)                                                                              \
    template void InvokeRobertsDown<typeSrc, typeDst>(                                                                 \
