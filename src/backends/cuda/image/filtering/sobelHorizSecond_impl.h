...
void InvokeSobelHorizSecond(const SrcT *aSrc1, size_t aPitchSrc1, DstT *aDst, size_t aPitchDst, MaskSize aMaskSize,
    ...
    if constexpr (oppEnablePixelType<DstT> && oppEnableCudaBackend<DstT>)
    {
        ...
        switch (aMaskSize)
        {
            case MaskSize::Mask_3x3:
            {
                ...
                using FixedFilterKernelT = FixedFilterKernel<opp::FixedFilter::SobelHorizSecond, filterSize, FilterT>;
                ...
                switch (aBorderType)
                {
                    ...
                    default:
                        throw INVALIDARGUMENT(
                            aBorderType,
                            aBorderType << " is not a supported border type mode for SobelHorizSecond filter.");
                        break;
                }
            }
            break;
            ...
            break;
            default:
                throw INVALIDARGUMENT(aMaskSize, "Invalid MaskSize for SobelHorizSecond filter: " << aMaskSize);
                break;
        }
    }
}
...
#define Instantiate_For(typeSrc, typeDst)                                                                              \
    template void InvokeSobelHorizSecond<typeSrc, typeDst>(                                                            \
