...
{
/// <summary>
/// Applies a separable filter to each pixel in an image with border control and then merges the original
...
template <int WarpAlignmentInBytes, int TupelSize, class ComputeT, class DstT, int blockHeight,
          RoundingMode roundingMode, typename BorderControlT, typename FilterT>
__global__ void unsharpFilterKernel(BorderControlT aSrcWithBC, DstT *__restrict__ aDst, size_t aPitchDst,
...
template <typename ComputeT, typename DstT, size_t TupelSize, int WarpAlignmentInBytes, int blockHeight,
          RoundingMode roundingMode, typename BorderControlT, typename FilterT>
void InvokeUnsharpFilterKernel(const dim3 &aBlockSize, uint aSharedMemory, int aWarpSize, cudaStream_t aStream,
                               const BorderControlT &aSrcWithBC, DstT *aDst, size_t aPitchDst, const FilterT *aFilter,
                               FilterT aWeight, FilterT aThreshold, int aFilterSize, int aFilterCenter,
                               const Size2D &aSize)
{
    ...
    unsharpFilterKernel<WarpAlignmentInBytes, TupelSize, ComputeT, DstT, blockHeight, roundingMode, BorderControlT,
                        FilterT><<<blocksPerGrid, aBlockSize, aSharedMemory, aStream>>>(
        aSrcWithBC, aDst, aPitchDst, aFilter, aWeight, aThreshold, aFilterSize, aFilterCenter, aSize, ts);
...
void InvokeUnsharpFilterKernelDefault(const BorderControlT &aSrcWithBC, DstT *aDst, size_t aPitchDst,
                                      const FilterT *aFilter, FilterT aWeight, FilterT aThreshold, int aFilterSize,
                                      int aFilterCenter, const Size2D &aSize, const mpp::cuda::StreamCtx &aStreamCtx)
{
    if (aStreamCtx.ComputeCapabilityMajor < INT_MAX)
    {
        ...
        {
            InvokeUnsharpFilterKernel<ComputeT, DstT, TupelSize, WarpAlignmentInBytes, blockHeight, roundingMode,
                                      BorderControlT, FilterT>(BlockSize, SharedMemory, aStreamCtx.WarpSize,
                                                               aStreamCtx.Stream, aSrcWithBC, aDst, aPitchDst, aFilter,
                                                               aWeight, aThreshold, aFilterSize, aFilterCenter, aSize);
        }
        else if (blockHeight > 1)
        {
            ...
            {
                throw INVALIDARGUMENT(aFilterSize, "Even with reduced launch configuration, the filter kernel size ("
                                                       << aFilterSize << " pixels) requires too much shared memory ("
                                                       << SharedMemory2 << " Bytes). The limit is "
                                                       << aStreamCtx.SharedMemPerBlock
                                                       << " Bytes - cannot launch kernel.");
            }
            InvokeUnsharpFilterKernel<ComputeT, DstT, TupelSize, WarpAlignmentInBytes, 1, roundingMode, BorderControlT,
                                      FilterT>(BlockSize, SharedMemory2, aStreamCtx.WarpSize, aStreamCtx.Stream,
                                               aSrcWithBC, aDst, aPitchDst, aFilter, aWeight, aThreshold, aFilterSize,
                                               aFilterCenter, aSize);
        }
        else
        {
            throw INVALIDARGUMENT(aFilterSize, "The filter kernel size ("
                                                   << aFilterSize << " pixels) requires too much shared memory ("
                                                   << SharedMemory << " Bytes). The limit is "
                                                   << aStreamCtx.SharedMemPerBlock << " Bytes - cannot launch kernel.");
        }
    }
    else
    {
        throw CUDAUNSUPPORTED(unsharpFilterKernel,
