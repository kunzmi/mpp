                     ...
                     size_t aPitchDst, const Size2D &aSize, const StreamCtx &aStreamCtx)
{
    ...
    if constexpr (simdOP_t::has_simd)
    {
        ...
        InvokeForEachPixelKernelDefault<DstT, TupelSize, addSrcSrcSIMD>(aDst, aPitchDst, aSize, aStreamCtx, functor);
    }
    else
    {
        // set to roundingmode NONE, because Add cannot produce non-integers in computations with ints:
                         ...
                         const StreamCtx &aStreamCtx)
{
    ...
    if constexpr (simdOP_t::has_simd)
    {
        ...
        InvokeForEachPixelKernelDefault<DstT, TupelSize, addInplaceSrcSIMD>(aSrcDst, aPitchSrcDst, aSize, aStreamCtx,
                                                                            functor);
    }
    else
    {
                          ...
                          const StreamCtx &aStreamCtx)
{
    ...
    InvokeForEachPixelKernelDefault<DstT, TupelSize, addInplaceDevC>(aSrcDst, aPitchSrcDst, aSize, aStreamCtx, functor);
}
...
template <typename SrcT, typename ComputeT, typename DstT>
void InvokeAddInplaceDevCScale(DstT *aSrcDst, size_t aPitchSrcDst, const SrcT *aConst, double aScaleFactor,
                               const Size2D &aSize, const StreamCtx &aStreamCtx)
{
    MPP_CUDA_REGISTER_TEMPALTE;
