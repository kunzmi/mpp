                           ...
                           const StreamCtx &aStreamCtx)
{
    ...
    if (aSizeSrc == sizeDst)
    {
        ...
        using transformerT = TransformerFunctor<1, DstT, CoordT, false, InterpolatorT, TransformerPStoFFTW<CoordT>,
                                                roundingMode, mpp::MagnitudeLog<ComputeT>>;
    ...
    else
    {
        ...
        using magnitudeLogSrc =
            SrcFunctor<TupelSize, SrcT, ComputeT, DstT, mpp::MagnitudeLog<ComputeT>, RoundingMode::NearestTiesToEven>;

        const mpp::MagnitudeLog<ComputeT> op(aOffset);

        const magnitudeLogSrc functor(aSrc1, aPitchSrc1, op);

        InvokeForEachPixelKernelDefault<DstT, TupelSize, magnitudeLogSrc>(aDst, aPitchDst, aSize, aStreamCtx, functor);
    }
}
...
#define Instantiate_For(typeSrc, typeDst)                                                                              \
    template void InvokeMagnitudeLogSrc<typeSrc, default_compute_type_for_t<typeSrc>, typeDst>(                        \
        const typeSrc *aSrc1, size_t aPitchSrc1, typeDst *aDst, size_t aPitchDst, remove_vector_t<typeDst> aOffset,    \
        const Size2D &aSizeSrc, const Size2D &aSize, const StreamCtx &aStreamCtx);
