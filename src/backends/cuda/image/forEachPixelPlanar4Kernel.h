...
{
/// <summary>
/// runs aFunctor on every pixel of an image. Inplace and outplace operation, no mask. Planar 4 channel destination, for
...
__global__ void forEachPixelPlanar4Kernel(Vector1<remove_vector_t<DstT>> *__restrict__ aDst1, size_t aPitchDst1,
                                          ...
                                          funcType aFunctor)
{
    ...
    if constexpr (TupelSize > 1) // evaluated at compile time
    {
        if (aSplit.ThreadIsAlignedToWarp(threadX))
        {
            ...
            DstPlaneT *pixelsOut4 = gotoPtr(aDst4, aPitchDst4, pixelX, pixelY);
...
#pragma unroll
                for (size_t i = 0; i < TupelSize; i++)
                {
                    res.value[i].w = resPlane.value[i].x;
                }
            }
...
#pragma unroll
    ...
    DstPlaneT *pixelOut4 = gotoPtr(aDst4, aPitchDst4, pixelX, pixelY);
                                     ...
                                     const Size2D &aSize, const funcType &aFunctor)
{
    ...
    forEachPixelPlanar4Kernel<WarpAlignmentInBytes, TupelSize, DstT, funcType>
        <<<blocksPerGrid, aBlockSize, aSharedMemory, aStream>>>(aDst1, aPitchDst1, aDst2, aPitchDst2, aDst3, aPitchDst3,
                                                                aDst4, aPitchDst4, aSize, ts, aFunctor);
...
void InvokeForEachPixelPlanar4KernelDefault(Vector1<remove_vector_t<DstT>> *__restrict__ aDst1, size_t aPitchDst1,
                                            Vector1<remove_vector_t<DstT>> *__restrict__ aDst2, size_t aPitchDst2,
                                            Vector1<remove_vector_t<DstT>> *__restrict__ aDst3, size_t aPitchDst3,
                                            Vector1<remove_vector_t<DstT>> *__restrict__ aDst4, size_t aPitchDst4,
                                            ...
                                            const funcType &aFunc)
{
    ...
    {
        throw CUDAUNSUPPORTED(forEachPixelPlanar4Kernel,
