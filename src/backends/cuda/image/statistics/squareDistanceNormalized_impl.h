...
#include <backends/cuda/image/squareDistanceNormalizedKernel.h>
...
void InvokeSquareDistanceNormalized(const SrcT *aSrc1, size_t aPitchSrc1, DstT *aDst, size_t aPitchDst,
    ...
    if constexpr (oppEnablePixelType<SrcT> && oppEnableCudaBackend<SrcT>)
    {
        ...
        switch (aBorderType)
        {
            case opp::BorderType::None:
            {
                ...
                InvokeSquareDistanceNormalizedKernelDefault<ComputeT, DstT, TupelSize, SrcT, pixelBlockSizeX,
            ...
            default:
                throw INVALIDARGUMENT(
                    aBorderType, aBorderType << " is not a supported border type mode for SquareDistanceNormalized.");
                break;
        }
    }
}
...
#define Instantiate_For(typeSrc, typeDst)                                                                              \
    template void InvokeSquareDistanceNormalized<typeSrc, typeDst>(                                                    \
