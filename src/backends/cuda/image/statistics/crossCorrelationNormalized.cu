...
#include <backends/cuda/image/crossCorrelationNormalizedKernel.h>
...
void InvokeCrossCorrelationNormalized(const SrcT *aSrc1, size_t aPitchSrc1, DstT *aDst, size_t aPitchDst,
                                      const SrcT *aTemplate, size_t aPitchTemplate, const Size2D &aSizeTemplate,
    ...
    if constexpr (oppEnablePixelType<SrcT> && oppEnableCudaBackend<SrcT>)
    {
        ...
        switch (aBorderType)
        {
            case opp::BorderType::None:
            {
                ...
                InvokeCrossCorrelationNormalizedKernelDefault<ComputeT, DstT, TupelSize, SrcT, pixelBlockSizeX,
            ...
            default:
                throw INVALIDARGUMENT(
                    aBorderType, aBorderType << " is not a supported border type mode for CrossCorrelationNormalized.");
                break;
        }
    }
}
...
#define Instantiate_For(typeSrc, typeDst)                                                                              \
    template void InvokeCrossCorrelationNormalized<typeSrc, typeDst>(                                                  \
