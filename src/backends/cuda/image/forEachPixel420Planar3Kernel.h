...
/// with 420 chroma sub-sampling.
/// </summary>
template <class DstT, typename funcType, ChromaSubsamplePos chromaSubsamplePos>
__global__ void forEachPixel420Planar3Kernel(Vector1<remove_vector_t<DstT>> *__restrict__ aDst1, size_t aPitchDst1,
                                             ...
                                             Size2D aSize, funcType aFunctor)
{
    ...
    const int pixelY       = threadY * subSampleSize;
    const int pixelYChroma = threadY;

    // skip uneven pixels at the border
    if (threadX >= aSize.x / subSampleSize || threadY >= aSize.y / subSampleSize)
    {
        return;
    }

    DstT res[subSampleSize * subSampleSize];
    DstPlane *pixelOutLuma1   = gotoPtr(aDst1, aPitchDst1, pixelX, pixelY);
    DstPlane *pixelOutLuma2   = gotoPtr(aDst1, aPitchDst1, pixelX, pixelY + 1);
    DstPlane *pixelOutChroma1 = gotoPtr(aDst2, aPitchDst2, pixelXChroma, pixelYChroma);
    DstPlane *pixelOutChroma2 = gotoPtr(aDst3, aPitchDst3, pixelXChroma, pixelYChroma);

    // load the destination pixel in case of inplace operation:
    static_assert(!funcType::DoLoadBeforeOp, "Pre-loading on 420 sub-sampled data is not implemented.");

    Tupel<DstPlane, subSampleSize> resLuma[subSampleSize];
#pragma unroll
...
#pragma unroll
        for (int sx = 0; sx < subSampleSize; sx++)
        {
            ...
            resLuma[sy].value[sx].x = res[sy * subSampleSize + sx].x;
        }
    }
    // maybe aligned or un-aligned:
    Tupel<DstPlane, subSampleSize>::Store(resLuma[0], pixelOutLuma1);
    Tupel<DstPlane, subSampleSize>::Store(resLuma[1], pixelOutLuma2);
    ...
    else if constexpr (chromaSubsamplePos == ChromaSubsamplePos::Left)
    {
        // average chroma values:
        if constexpr (RealFloatingVector<DstT>)
        {
...
void InvokeForEachPixel420Planar3Kernel(const dim3 &aBlockSize, cudaStream_t aStream,
                                        ...
                                        ChromaSubsamplePos chromaSubsamplePos)
{
    ...
    {
        forEachPixel420Planar3Kernel<DstT, funcType, ChromaSubsamplePos::Center>
            <<<blocksPerGrid, aBlockSize, 0, aStream>>>(aDst1, aPitchDst1, aDst2, aPitchDst2, aDst3, aPitchDst3, aSize,
                                                        aFunctor);
    }
    else if (chromaSubsamplePos == ChromaSubsamplePos::Left)
    {
        forEachPixel420Planar3Kernel<DstT, funcType, ChromaSubsamplePos::Left>
            <<<blocksPerGrid, aBlockSize, 0, aStream>>>(aDst1, aPitchDst1, aDst2, aPitchDst2, aDst3, aPitchDst3, aSize,
                                                        aFunctor);
    }
    else
    {
        forEachPixel420Planar3Kernel<DstT, funcType, ChromaSubsamplePos::TopLeft>
            <<<blocksPerGrid, aBlockSize, 0, aStream>>>(aDst1, aPitchDst1, aDst2, aPitchDst2, aDst3, aPitchDst3, aSize,
                                                        aFunctor);
    }
...
template <typename DstT, typename funcType>
void InvokeForEachPixel420Planar3KernelDefault(Vector1<remove_vector_t<DstT>> *__restrict__ aDst1, size_t aPitchDst1,
                                               Vector1<remove_vector_t<DstT>> *__restrict__ aDst2, size_t aPitchDst2,
                                               Vector1<remove_vector_t<DstT>> *__restrict__ aDst3, size_t aPitchDst3,
                                               ...
                                               const mpp::cuda::StreamCtx &aStreamCtx, const funcType &aFunc)
{
    if (aStreamCtx.ComputeCapabilityMajor < INT_MAX)
    {
        ...
        InvokeForEachPixel420Planar3Kernel<DstT, funcType>(BlockSize, aStreamCtx.Stream, aDst1, aPitchDst1, aDst2,
                                                           aPitchDst2, aDst3, aPitchDst3, aSize, aFunc,
                                                           chromaSubsamplePos);
    }
    else
    {
        throw CUDAUNSUPPORTED(forEachPixel420Planar3Kernel,
