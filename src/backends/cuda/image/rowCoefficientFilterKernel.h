...
{
/// <summary>
/// Applies a row filter to each pixel in an image with border control.<para/>
/// </summary>
...
__global__ void rowCoefficientFilterKernel(BorderControlT aSrcWithBC, DstT *__restrict__ aDst, size_t aPitchDst,
...
#pragma unroll
            for (int kx = 0; kx < TupelSize; kx++)
            {
                ...
                for (int i = 0; i < aFilterSize; i++)
                {
                    ...
                    bufferReg[kx] += bufferLine[idx] * aFilter[i];
                }
            }
...
#pragma unroll
        ...
        if (pixelX >= 0 && pixelX < aSize.x)
        {
            ...
            for (int i = 0; i < aFilterSize; i++)
            {
                ...
                temp += buffer[idxShared] * aFilter[i];
            }
...
template <typename ComputeT, typename DstT, size_t TupelSize, int WarpAlignmentInBytes, RoundingMode roundingMode,
          typename BorderControlT, typename FilterT, bool needsScaling>
void InvokeRowCoefficientFilterKernel(const dim3 &aBlockSize, uint aSharedMemory, int aWarpSize, cudaStream_t aStream,
                                      ...
                                      int aFilterCenter, const Size2D &aSize)
{
    ...
    rowCoefficientFilterKernel<WarpAlignmentInBytes, TupelSize, ComputeT, DstT, roundingMode, BorderControlT, FilterT,
                               needsScaling><<<blocksPerGrid, aBlockSize, aSharedMemory, aStream>>>(
...
template <typename ComputeT, typename DstT, size_t TupelSize, RoundingMode roundingMode, typename BorderControlT,
          typename FilterT, bool needsScaling>
void InvokeRowCoefficientFilterKernelDefault(const BorderControlT &aSrcWithBC, DstT *aDst, size_t aPitchDst,
                                             const FilterT *aFilter, FilterT aScalingValueInv, int aFilterSize,
                                             int aFilterCenter, const Size2D &aSize,
                                             const opp::cuda::StreamCtx &aStreamCtx)
{
    if (aStreamCtx.ComputeCapabilityMajor < INT_MAX)
    {
        ...
        InvokeRowCoefficientFilterKernel<ComputeT, DstT, TupelSize, WarpAlignmentInBytes, roundingMode, BorderControlT,
                                         FilterT, needsScaling>(BlockSize, SharedMemory, aStreamCtx.WarpSize,
                                                                aStreamCtx.Stream, aSrcWithBC, aDst, aPitchDst, aFilter,
                                                                aScalingValueInv, aFilterSize, aFilterCenter, aSize);
    }
    else
    {
        throw CUDAUNSUPPORTED(rowCoefficientFilterKernel,
