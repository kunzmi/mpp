...
__global__ void reductionMaskedMaxIdxAlongXKernel(const byte *__restrict__ aMask, size_t aPitchMask, const SrcT *aSrc,
                                                  size_t aPitchSrc, SrcT *__restrict__ aDstMax,
                                                  ...
                                                  Size2D aSize, ThreadSplit<WarpAlignmentInBytes, TupelSize> aSplit)
{
    ...
    else
    {
             ...
             pixelXWarp0 += warpSize)
        {
            ...
            functor(pixelX, pixelY, resultMax, resultMaxIdx, maskTupel);
        }
...
void InvokeReductionMaskedMaxIdxAlongXKernel(const dim3 &aBlockSize, uint aSharedMemory, int aWarpSize,
                                             ...
                                             same_vector_size_different_type_t<SrcT, int> *aDstMaxIdx, Size2D aSize)
{
    ...
    reductionMaskedMaxIdxAlongXKernel<WarpAlignmentInBytes, TupelSize>
        <<<blocksPerGrid, aBlockSize, aSharedMemory, aStream>>>(aMask, aPitchMask, aSrc, aPitchSrc, aDstMax, aDstMaxIdx,
                                                                aSize, ts);
                                                    ...
                                                    const Size2D &aSize, const mpp::cuda::StreamCtx &aStreamCtx)
{
    if (aStreamCtx.ComputeCapabilityMajor < INT_MAX)
    {
        ...
        InvokeReductionMaskedMaxIdxAlongXKernel<SrcT, TupelSize, WarpAlignmentInBytes>(
            BlockSize, SharedMemory, aStreamCtx.WarpSize, aStreamCtx.Stream, reinterpret_cast<const byte *>(aMask),
            aPitchMask, aSrc, aPitchSrc, aDstMax, aDstMaxIdx, aSize);
    }
    else
    {
        throw CUDAUNSUPPORTED(reductionMaskedMaxIdxAlongXKernel,
