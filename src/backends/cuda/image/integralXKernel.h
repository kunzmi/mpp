...
#pragma unroll
    ...
    {
        temp = __shfl_up_sync(0xFFFFFFFF, value.value[TupelSize - 1].y, 1);
        if (warpLaneID >= 1)
        {
#pragma unroll
            for (size_t i = 0; i < TupelSize; i++)
            {
                value.value[i].y += temp;
            }
        }

        temp = __shfl_up_sync(0xFFFFFFFF, value.value[TupelSize - 1].y, 2);
        if (warpLaneID >= 2)
        {
#pragma unroll
            for (size_t i = 0; i < TupelSize; i++)
        ...
        temp = __shfl_up_sync(0xFFFFFFFF, value.value[TupelSize - 1].y, 4);
        if (warpLaneID >= 4)
        {
#pragma unroll
            for (size_t i = 0; i < TupelSize; i++)
        ...
        temp = __shfl_up_sync(0xFFFFFFFF, value.value[TupelSize - 1].y, 8);
        if (warpLaneID >= 8)
        {
#pragma unroll
            for (size_t i = 0; i < TupelSize; i++)
        ...
        temp = __shfl_up_sync(0xFFFFFFFF, value.value[TupelSize - 1].y, 16);
        if (warpLaneID >= 16)
        {
#pragma unroll
            for (size_t i = 0; i < TupelSize; i++)
    ...
    {
        temp = __shfl_up_sync(0xFFFFFFFF, value.value[TupelSize - 1].z, 1);
        if (warpLaneID >= 1)
        {
#pragma unroll
            for (size_t i = 0; i < TupelSize; i++)
            {
                value.value[i].z += temp;
            }
        }

        temp = __shfl_up_sync(0xFFFFFFFF, value.value[TupelSize - 1].z, 2);
        if (warpLaneID >= 2)
        {
#pragma unroll
            for (size_t i = 0; i < TupelSize; i++)
        ...
        temp = __shfl_up_sync(0xFFFFFFFF, value.value[TupelSize - 1].z, 4);
        if (warpLaneID >= 4)
        {
#pragma unroll
            for (size_t i = 0; i < TupelSize; i++)
        ...
        temp = __shfl_up_sync(0xFFFFFFFF, value.value[TupelSize - 1].z, 8);
        if (warpLaneID >= 8)
        {
#pragma unroll
            for (size_t i = 0; i < TupelSize; i++)
        ...
        temp = __shfl_up_sync(0xFFFFFFFF, value.value[TupelSize - 1].z, 16);
        if (warpLaneID >= 16)
        {
#pragma unroll
            for (size_t i = 0; i < TupelSize; i++)
    ...
    {
        temp = __shfl_up_sync(0xFFFFFFFF, value.value[TupelSize - 1].w, 1);
        if (warpLaneID >= 1)
        {
#pragma unroll
            for (size_t i = 0; i < TupelSize; i++)
            {
                value.value[i].w += temp;
            }
        }

        temp = __shfl_up_sync(0xFFFFFFFF, value.value[TupelSize - 1].w, 2);
        if (warpLaneID >= 2)
        {
#pragma unroll
            for (size_t i = 0; i < TupelSize; i++)
        ...
        temp = __shfl_up_sync(0xFFFFFFFF, value.value[TupelSize - 1].w, 4);
        if (warpLaneID >= 4)
        {
#pragma unroll
            for (size_t i = 0; i < TupelSize; i++)
        ...
        temp = __shfl_up_sync(0xFFFFFFFF, value.value[TupelSize - 1].w, 8);
        if (warpLaneID >= 8)
        {
#pragma unroll
            for (size_t i = 0; i < TupelSize; i++)
        ...
        temp = __shfl_up_sync(0xFFFFFFFF, value.value[TupelSize - 1].w, 16);
        if (warpLaneID >= 16)
        {
#pragma unroll
            for (size_t i = 0; i < TupelSize; i++)
...
/// <summary>
/// performs integral image computation in X direction and pads a row and a column with init value.
        ...
        for (int pixelXWarp0 = 0; pixelXWarp0 < aSize.x; pixelXWarp0 += warpSize)
        {
            ...
            if (pixelY > 0)
            {
                if (pixelX < aSize.x)
                {
                    if (pixelX > 0)
                    {
                        // pixelX - 1 do add 0 border at the left and top:
