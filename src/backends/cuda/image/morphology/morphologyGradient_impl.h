...
#include <backends/cuda/image/morphologyGradientKernel.h>
...
void InvokeMorphologyGradient(const SrcT *aSrc1, size_t aPitchSrc1, DstT *aDst, size_t aPitchDst,
                              const Pixel8uC1 *aMask, const FilterArea &aFilterArea, BorderType aBorderType,
                              ...
                              const mpp::cuda::StreamCtx &aStreamCtx)
{
    ...
    switch (aBorderType)
    {
        case mpp::BorderType::None:
        {
            ...
            InvokeMorphologyGradientKernelDefault<DstT, TupelSize, FilterT, pixelBlockSizeX, pixelBlockSizeY, BCType>(
                bc, aDst, aPitchDst, aMask, aFilterArea, aSize, aStreamCtx);
        }
        break;
        ...
        default:
            throw INVALIDARGUMENT(aBorderType,
                                  aBorderType << " is not a supported border type mode for Morphology Gradient.");
            break;
    }
}
...
#define Instantiate_For(typeSrc, typeDst)                                                                              \
    template void InvokeMorphologyGradient<typeSrc, typeDst>(                                                          \
