...
template <typename T> struct Neg
{
    ...
    DEVICE_ONLY_CODE void operator()(const T &aSrc1, T &aDst) const
    ...
    DEVICE_ONLY_CODE void operator()(T &aSrcDst) const
        requires std::same_as<T, sbyte1_8> || std::same_as<T, sbyte2_4>
    {
...
template <typename TTo> struct Convert
{
    ...
    DEVICE_ONLY_CODE void operator()(const Tupel<Vector1<float>, 4> &aSrc1, TTo &aDst) const
